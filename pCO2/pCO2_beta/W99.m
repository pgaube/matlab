%calculates the CO2-flux based off of the WM99 cubic dependance of flux on
%windspeed
...
for m = 1:length(lat)
    for n = 1:length(lon)
        ...
        K(m,n,p) = 3.3+0.026*(ncep_wind_spd(m,n,p))^3;  %Dependance of flux on windspeed bassed off of Magillis and Edson et al 2001 (cm/h)
        
        %K(m,n,p) = (K(m,n,p)^.5)*(S(m,n,p)/660);
        
        A(m,n,p) = a + (b * (100/sst(m,n,p))) + ...
                    (c * log(sst(m,n,p)/100)) + ...
                    (sal(m,n,p)*(d + (e * (sst(m,n,p)/100)) + ...
                    ((f * (sst(m,n,p)/100)^2))));  %Solubility of CO2 in Sea water based off of Weiss 1974 (mol/kg*atm)
        A(m,n,p) = exp(A(m,n,p));
