...
T_c=2*pi*(Ls*ri)./azav_v/60/60/24;
...
nazav_t=nazav_l+nazav_n+nazav_b;


figure(1)
clf
set(gcf,'PaperPosition',[1 1 15 10])
XXX=L/1e3*[ri;ri;ri;ri];
YYY=[nazav_l;nazav_n;nazav_b;nazav_t];
[ax,h1,h2]=plotyy(XXX',YYY',Ls/1e3*ri,T_c);
set(h1(1),'color','r','linewidth',3)
...
legend('W_c','W_\zeta','W_{\beta}','W_{tot}','rotational period','location','northwest')
ylabel('m','fontsize',25)
xlabel({'distance from eddy centroid (km)'},'fontsize',25)
title({'Net Vertical Displacment'},'fontsize',25)
set(ax,'xlim',[0 115])
axes(ax(1));
line([0 200],[0 0],'color',[.5 .5 .5],'linewidth',1)
axes(ax(2))
ylabel('rotational period (days)','fontsize',25)
set(ax(2),'ylim',[10 60],'ytick',[10:5:80],'xlim',[0 115])
set(ax(1),'ylim',[-.5 4.5],'ytick',[-1:.5:5],'xlim',[0 115],'YAxisLocation','left')

print -dpng -r300 figs/azav_terms_with_zeta_scale.png
% !open figs/azav_terms_with_zeta_scale.png
return
%}
%now vary L
