...
head={'GAUBE NH ROTATED','GAUBE SH ROTATED','GAUBE GLOB ROTATED'};
out={'GAUBE_wind_NH_comps_diff','GAUBE_wind_SH_comps_diff','GAUBE_wind_midlat_comps_diff'};
mult=[1 1 1];
for m=1:3
eval(['load ',gaube{m},' w*'])
...
%%tilde w_tot_ac
imag=double(interp2(w_ek_total_a.mean-((.013/.01)*w_ek_sst_fixed_a.mean)));
...
%%diff ac
dd=(mult(m)*w_ek_total_qscat_a.mean)-(w_ek_total_a.mean-((.013/.01)*w_ek_sst_fixed_a.mean));
...
%%tilde w_tot_cc
imag=double(interp2(w_ek_total_c.mean-(.013/.01)*w_ek_sst_fixed_c.mean));
...
dd=(mult(m)*w_ek_total_qscat_c.mean)-(w_ek_total_c.mean-(.013/.01)*w_ek_sst_fixed_c.mean);
imag=double(interp2(dd));
...
% print -dpng -r300 /Users/new_gaube/Documents/Publications/gaube_chelton_sst_wind_eddies/figs/schalx_compare/gaube_final_figs/GAUBE_wind_midlat_comps_diff
end