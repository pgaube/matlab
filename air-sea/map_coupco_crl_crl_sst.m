...
CRLS=single(nan(560,1440,lj));
CRL=CRLS;


for m=1:lj
	fprintf('\r    loading %03u of %03u\r',m,lj)
	load([qsave_path qsave_head num2str(jdays(m))],'mask','bp26_crl','bp26_crl_sst')
	
	CRL(:,:,m)=single(bp26_crl);
	CRLS(:,:,m)=single(bp26_crl_sst);
	clear bp26*
end

save -v7.3 all_crl_crl_sst lat lon CRL CRLS year month day jdays

%now do coupco

%}
load all_crl_crl_sst
...
save coupco_map_crl_crl_sst beta_* CI_* lat lon sm_coupco_*
