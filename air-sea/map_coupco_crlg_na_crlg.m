...
for m=1:lj
	...
	load([RAND_PATH RAND_HEAD num2str(jdays(m))],'na_crlg')
	load([asave_path asave_head num2str(jdays(m))],'bp26_crlg','ls_mask')
	mask=ls_mask;
	
	CRL(:,:,m)=single(na_crlg.*mask);
	CRLG(:,:,m)=single(bp26_crlg.*mask);
	clear bp26* na*
end

save -v7.3 all_na_crlg lat lon CRL CRLG year month day jdays
...
save coupco_map_na_crlg beta_* CI_* lat lon sm_coupco_*
