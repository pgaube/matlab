...
for m=1:lj
	...
	load([qsave_path qsave_head num2str(jdays(m))],'bp26_crl','mask')
	cmask=mask;
	load([asave_path asave_head num2str(jdays(m))],'bp26_crlg','ls_half_mask')
	mask=ls_half_mask(rs,:);
	
	CRL(:,:,m)=single(bp26_crl.*mask);
	CRLG(:,:,m)=single(bp26_crlg(rs,:).*mask);
	clear bp26*
end

save -v7.3 all_crls_mask_half_mask lat lon CRL CRLG year month day jdays
return
%now do coupco
...
for m=1:560
	...
	for n=1:1440
		...
		if(~isnan(beta_n(m,n)))
			beta_ols(m,n)=ols_line(double(x),double(y),double(beta_n(m,n)));
		end
		if Cor>Sig
			CI_n(m,n)=1;
		end	
...
sm_coupco_ols=smoothn(beta_ols,20);
