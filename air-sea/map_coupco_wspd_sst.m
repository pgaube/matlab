...
for m=1:lj
	...
	SST(:,:,m)=single(bp26_sst(ro,:)).*mask(rs,:);
	WSPD(:,:,m)=single(bp26_wspd).*mask(rs,:);
	clear bp26*
end

save -v7.3 all_wspd_sst lat lon SST WSPD year month day jdays
return
%now do coupco
...
save coupco_map_wspd_sst beta_* CI_* lat lon sm_coupco_*
