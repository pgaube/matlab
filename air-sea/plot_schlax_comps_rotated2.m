...
out={'SCHLAX_wind_NH_comps','SCHLAX_wind_SH_comps','SCHLAX_wind_midlat_comps'};
mult=[.94 .98 1.2];
for m=1:3
...
%%w_cur_ac
imag=double(.95*interp2(w_ek_total_a.mean));
tit=['W_{cur} AC N=',num2str(w_ek_total_a.n_max_sample)];
...
%%w_sst_ac
imag=double(.91*interp2((.013/.01)*w_ek_sst_fixed_a.mean));
...
%%w_cur_ac
imag=double(.95*interp2(w_ek_total_c.mean));
tit=['W_{cur} CC N=',num2str(w_ek_total_c.n_max_sample)];
...
%%w_sst_ac
imag=double(.91*interp2((.013/.01)*w_ek_sst_fixed_c.mean));
...
% print -dpng -r300 /Users/new_gaube/Documents/Publications/gaube_chelton_sst_wind_eddies/figs/schalx_compare/gaube_final_figs/SCHLAX_wind_midlat_comps
end