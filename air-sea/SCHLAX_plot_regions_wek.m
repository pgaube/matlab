...
mult_tot=[1     1 .87 .9  .8 .9];
mult_cur=[.98 .95 .95 .90 .9 .95];
mult_sst=[.96 .95 .95 .9  .95 .87];
...
load GAUBE_region_comps *_wek_total_qscat_*


for mm=1:6
eval(['w_tot_a=mult_tot(mm)*interp2(',curs{mm},'_wek_total_qscat_a.mean);'])
eval(['w_tot_c=mult_tot(mm)*interp2(',curs{mm},'_wek_total_qscat_c.mean);'])

eval(['w_cur_a=mult_cur(mm)*interp2(',curs{mm},'_wek_total_a.mean);'])
eval(['w_cur_c=mult_cur(mm)*interp2(',curs{mm},'_wek_total_c.mean);'])

eval(['w_sst_a=-mult_sst(mm)*interp2((.013/.01)*',curs{mm},'_fixed_wek_sst_a.mean);'])
eval(['w_sst_c=-mult_sst(mm)*interp2((.013/.01)*',curs{mm},'_fixed_wek_sst_c.mean);'])
...
ylabel(tit,'fontsize',12,'fontweight','bold')
eval(['print -dpng -r300 /Users/new_gaube/Documents/Publications/gaube_chelton_sst_wind_eddies/figs/schalx_compare/gaube_final_figs/SCHLAX_',curs2{mm},'_comps'])
