...
for m=1:lj
	...
	load([qsave_path qsave_head num2str(jdays(m))],'mask','hp66_crl','bp26_dtdn','bp26_dtds')
	qs_mask=mask;
	load([asave_path asave_head num2str(jdays(m))],'hp66_crlg','ls_5_cm_mask')
	mask=ls_5_cm_mask;
	CRL(:,:,m)=single(hp66_crl.*qs_mask.*mask(rs,:));
	CRLG(:,:,m)=single(hp66_crlg(rs,:).*qs_mask.*mask(rs,:));
	clear bp26* hp66*
end

save -v7.3 all_hp_5_cm_crls lat lon CRL CRLG year month day jdays

%now do coupco

%}
load all_hp_5_cm_crls
...
save coupco_map_hp_crls beta_* CI_* lat lon sm_coupco_*
