...
load obs_stream_sla
vars={'obs_ssh','norm_hp66_chl','chl','mean_chl','hp66_chl','norm_hp66_c'};

orgin_vars={'chl_orgin','hp_chl_orgin'};
figure(1)
clf
...
box
print -dpng -r300 figs/stream_obs_ts/n_ts

for m=5%3:5%1:length(vars)
...
%     if m==2
%         ii=find(norm_hp66_chl_a.id==415)
%         plot(norm_hp66_chl_a.k(ii),norm_hp66_chl_a.ks_mean_all(ii),'r','linewidth',2)
%         ii=find(norm_hp66_chl_c.id==324)
%         plot(norm_hp66_chl_c.k(ii),norm_hp66_chl_c.ks_mean_all(ii),'b','linewidth',2)
%     end
    axis tight
    ...
    eval(['print -dpng -r300 figs/stream_obs_ts/',vars{m},'_ts'])
end
% 
...
%         tt=hp66_chl_a.ks_orgin_all(ii);
%         tt(2:end)=tt(2:end)./10;
%         plot(hp66_chl_a.k(ii),tt,'r','linewidth',2)
%         ii=find(hp66_chl_c.id==324)
%         tt=norm_hp66_chl_c.ks_orgin_all(ii);
%         tt(2:end)=tt(2:end)./10;
%         plot(norm_hp66_chl_c.k(ii),tt,'b','linewidth',2)
%     end
    axis tight
    ...
    eval(['print -dpng -r300 figs/stream_obs_ts/orgin_',vars{m},'_ts'])
end
...
title('anticyclones')
set(gca,'ylim',[.1 .7],'xlim',[1 16],'fontsize',10,'fontweight','bold','LineWidth',.5,'TickLength',[.01 .05],'layer','top')
