...
%     patch(xp,xts_c,[.8 .8 .8])
    ...
    box
    print -dpng -r300 figs/meander_obs_ts/n_ts
    
for m=2:5%1:length(vars)
...
%         patch(xp,xts_c,[.8 .8 .8])
    ...
    if m==2
        set(gca,'ylim',[-.8 .8],'ytick',[-1:.2:1])
    end
    eval(['print -dpng -r300 figs/meander_obs_ts/',vars{m},'_ts'])
    end


for m=3:5%:length(orgin_vars)
...
%         patch(xp,xts_c,[.8 .8 .8])
    ...
    if m==2
        set(gca,'ylim',[-.5 .5],'ytick',[-1:.1:1])
    end
    eval(['print -dpng -r300 figs/meander_obs_ts/orgin_',vars{m},'_ts'])
end
