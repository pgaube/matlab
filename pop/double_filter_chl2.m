...
for m=1:length(pdays)
    m
    ttt=griddata(lon,lat,CHL(:,:,m),glon,glat,'linear');
...
%     drawnow
%     
    eval(['save -append mat/run14_',num2str(pdays(m)),' hp66_chl'])
    clear hp66_chl;
end
save -append tmp_chl hp_CHL
...
for m=1:length(lat(:,1))
    for n=1:length(lon(1,:))
        tmp=squeeze(hp_CHL(m,n,:));
        ss_CHL(m,n,:)=harm_reg(tmp_jdays,tmp,2,2*pi/365.25);
    end
end

ds_CHL=hp_CHL-ss_CHL;

save -append tmp_chl tmp_jdays CHL ss_CHL ds_CHL
for m=1:length(pdays)
    m
    ss66_chl=ds_CHL(:,:,m);
