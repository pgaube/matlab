...
load GS_rings_tracks_run14_sla pop_eddies pdays
...
for m=1:length(pdays)
    if exist([spath,num2str(pdays(m)),'.mat'])
        ...
        if exist('bp21_ssh')
            ...
            ii=find(pop_eddies.jday==pdays(m))
...
save pop_hp66_ssh_and_hp66_chl_by_polarity plat plon pdays CHL SSH CHL_a CHL_c CHL_t CHL_n

load pop_hp66_ssh_and_hp66_chl_by_polarity
lags=-5:5;
...
save pop_bec_cor_0 *r0 *N plon plat