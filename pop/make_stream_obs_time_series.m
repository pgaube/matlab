...
load obs_stream_sla
vars={'obs_ssh','norm_hp66_chl','norm_hp66_c'};

orgin_vars={'chl_orgin','hp_chl_orgin'};

for m=2%1:length(vars)
    
    eval(['ts_a=',vars{m},'_a.ks_05(1:tend);'])
    eval(['std_ts_a=',vars{m},'_a.ks_std2_05(1:tend);'])
    eval(['n_ts_a=',vars{m},'_a.ks_n(1:tend);'])
    
    eval(['ts_c=',vars{m},'_c.ks_05(1:tend);'])
    eval(['std_ts_c=',vars{m},'_c.ks_std2_05(1:tend);'])
...
%     patch(xp,xts_c,[.8 .8 .8])
    ...
    box
    title(orgin_vars{m})
    eval(['print -dpng -r300 figs/stream_obs_ts/',orgin_vars{m},'_ts'])
end
return
