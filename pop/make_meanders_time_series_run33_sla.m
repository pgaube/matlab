...
vars={'ssh','norm_hp66_chl','chl','hp66_chl','norm_hp66_c','w','tno3',...
...
%     patch(xp,xts_c,[.8 .8 .8])
    ...
    box
    print -dpng -r300 figs/meanders_ts_run33/n_ts
    
for m=2:4%length(vars)
...
%         patch(xp,xts_c,[.8 .8 .8])
    ...
    if m==2
        set(gca,'ylim',[-2 2],'ytick',[-2:.5:2])
    end
    
    eval(['print -dpng -r300 figs/meanders_ts_run33/',vars{m},'_ts'])
end


for m=2:4%:length(orgin_vars)
...
%         patch(xp,xts_c,[.8 .8 .8])
    ...
    eval(['print -dpng -r300 figs/meanders_ts_run33/orgin_',vars{m},'_ts'])
end
return
% 
