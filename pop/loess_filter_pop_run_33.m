...
fname1='/private/d1/larrya/public_html/0pt1/output_run33/t.33.'; %without eddy wind
...
fid=fopen('/private/d1/larrya/public_html/0pt1/grid/grid.992x1280.da','r','ieee-be');
...
for nn=vv; % time
    ...
    if exist([fname1 flid '.ssh'])
        ...
        ttt=griddata(lon,lat,ssh,glon,glat,'linear');
        tic
        sm=smooth2d_loess(ttt,glon(1,:),glat(:,1),2,2,glon(1,:),glat(:,1));
        lp=smooth2d_loess(sm,glon(1,:),glat(:,1),20,10,glon(1,:),glat(:,1));
        toc
        lph=griddata(glon,glat,lp,lon,lat','linear');
        smh=griddata(glon,glat,sm,lon,lat','linear');
        bp21_ssh=smh-lph;
        
        eval(['save -append mat/run33_',num2str(nn),' bp21_ssh'])
    end
