...
for m=[28]%:length(lat)
	load(['gline_',num2str(m),'_hov'],'chov','slon','shov')
    ...
    eval(['print -dpng -r300 figs/cor_cs_' num2str(m)])
    
    
	%eval(['print -dpng -r300 figs/cor_' num2str(m)])
	%eval(['save -append gline_' num2str(m) '_hov cs cc ss'])
end
