...
fSST = nan(length(mlat(:,1)),length(mlon(1,:)),length(tmp(:,1)));
...
for m=1:length(tmp)
	...
	if fname(23:26)=='SST.' | fname(23:26)=='SST4'
		TYPE='SST'
		...
		if length(r)>0 & length(c)>0
			...
			fSST(:,:,m) = griddata(lon,lat,full_sst,mlon,mlat,'linear');
			wSST(:,:,m) = griddata(lon,lat,wsst,mlon,mlat,'linear');
		end	
		end
	end	
end
...
save([MAT_DIR,'pgSoCal_modisa_sst_',num2str(s(1)) num2str(d)])
...
for m=1:length(SST(1,1,:))
	...
	tt
	fplot((9/5)*squeeze(SST(:,:,m))+31,mask,ran(1),ran(2)-1,'fdcolor',[IMAGE_DIR 'bdsc_'],pd,tt,'sst')
	fplot((9/5)*squeeze(fSST(:,:,m))+31,mask,ran(1),ran(2)-1,'fdcolor',[FULL_IMAGE_DIR 'bdsc_'],pd,tt,'sst')
	fplot((9/5)*squeeze(wSST(:,:,m))+31,mask,ran(1),ran(2)-1,'fdcolor',[IMAGE_DIR3 'bdsc_'],pd,tt,'sst')
end
...
fplot((9/5)*fsst+31,mask,ran(1),ran(2)-1,'fdcolor',[FULL_IMAGE_DIR 'bdsc_'],pd,tt,'sst')
fplot((9/5)*wsst+31,mask,ran(1),ran(2)-1,'fdcolor',[FULL_IMAGE_DIR 'bdsc_'],pd,tt,'sst')
