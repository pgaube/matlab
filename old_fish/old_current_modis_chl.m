...
for m=1:length(tmp)
	...
	if time(m)./100<12
		...
		tt=[num2str(time(m)-1200) 'p'];
	end	
	%plot each pass
	fplot(log10(CHL(:,:,m)),mask,cran(1),cran(2),'fdcolor',[IMAGE_DIR 'chl_bdsc_'],pd,tt,'chl')
end
...
fplot(log10(chl),mask,cran(1),cran(2),'fdcolor',[L0_IMAGE_DIR 'chl_bdsc_'],pd,tt,'chl')
fplot(log10(chl),mask,cran(1),cran(2),'fdcolor',[IMAGE_DIR 'chl_bdsc_'],pd,tt,'chl')
