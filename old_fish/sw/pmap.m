...
%colormap('jet');
m_grid('xtick',[round(min_lon):1:round(max_lon)],'ytick',[round(min_lat):.5:round(max_lat)],...
'tickdir','in','color','k','lineweight',1);  
hm=gca;
set(hm,'clipping','off')
hold on
%end
...
case {'sf_domain'}
    %clf
    %m_proj('UTM','lon',[min_lon max_lon],'lat',[min_lat max_lat]);   
	%m_grid('xtick',[round(min_lon):round(max_lon)],'ytick',[round(min_lat):round(max_lat)],...
	%'tickdir','in','color','k','lineweight',1.5);  
...
case {'tracks_dots'}
   ...
   for m=1:length(cunique)
        r=find(id(cid)==cunique(m));
        m_plot(cx(r),cy(r),'b.','markersize',2)
   end
    for m=1:length(aunique)
         r=find(id(aid)==aunique(m));
         m_plot(ax(r),ay(r),'r.','markersize',2)
    end
    %m_grid('xtick',[round(min_lon):3:round(max_lon)],'ytick',[round(min_lat): ...
    %        2:round(max_lat)],'tickdir','out','color','k');
    grid
...
case {'clshpssh'}
 ...
 case {'ssh'}
    ...
    caxis([-15 15])
     ...
     %m_text(287.5,-26.2,'Negative Contours Dashed   ','fontsize',8)
     grid
     title({'Sea Surface Height',dlabel});
     m_text(282,-25.7,'Contour Interval 2 cm, Negative Contours Dashed  ','fontsize',8)
     %axes(c);
     set(c,'position',[.15 .015 .74 .025]);
     %xlabel('cm')
     hold off
