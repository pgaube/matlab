...
for m=1:length(lat)
	...
	[cghov,hp_cghov,full_cghov,cglon,cgyear_day,cgjdays]=hovmoller([wlon(m) elon(m)],lat(m),'bp26_crlg');
	[nahov,hp_nahov,full_nahov,nalon,nayear_day,najdays]=hovmoller([wlon(m) elon(m)],lat(m),'na_crlg');
	[qshov,hp_nahov,full_qshov,nalon,nayear_day,najdays]=hovmoller([wlon(m) elon(m)],lat(m),'bp26_crl');
	...
	%}
	if exist(['line_',num2str(m),'_hov.mat'])
    eval(['save -append line_',num2str(m),'_hov'])
    else
    eval(['save line_',num2str(m),'_hov'])
    end
