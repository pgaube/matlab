...
shead='AVISO_25_W_';
sdir='/Volumes/matlab/data/eddy/V4/mat/'
%Set range of dates
...
for m=1:length(jdays)
	...
	load([sdir shead num2str(jdays(m))],'a_mask')
	mask=a_mask(41:600,:);
	w(:,:,m)=w_ek.*mask;
	%figure(1)
	%clf
	%pcolor(double(w_ek.*mask));shading flat;caxis([-.3 .3])
	%drawnow
end

w_bar.mean=nanmean(w,3);
w_bar.median=nanmedian(w,3);
w_bar.sm_median=linx_smooth2d_f(w_bar.median,3,3);
...
gw_bar.sm_median=biggrid(w_bar.sm_median,25,1);
...
dNdz=squeeze((N_bar.mean(1,:,:)-N_bar.mean(14,:,:))./500); %mmol N m^{-1}

k=10^-4; %m^s s^-1
lat=lat(r);
lon=lon(c);

F = gw_bar.sm_median.*squeeze(N_bar.mean(10,:,:)) - (86400*k*dNdz);


save woa_N_flux F lat lon gw_bar N_bar w_bar
return
