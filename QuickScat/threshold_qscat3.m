...
for m=1:length(jdays)
    ...
    load([rpath num2str(jdays(m))],'wek','hp_wek_sst_week_dtdn','hp_wek_sst_week_fixed','hp_wek_crlg_week')
    ...
    if ~exist('hp_wek_sst_week_fixed')
        hp_wek_sst_week_fixed=nan*wek;
    end
...
%     tc(isnan(hp_wek_sst_week_dtdn))=nan;
    ...
    tt=find(abs(rm.*hp_wek_sst_week_fixed)>sst_th);
    tw(tt)=nan;tc(tt)=2;
