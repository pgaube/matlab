...
for m=1:length(jdays)
    ...
    load([spath num2str(jdays(m))],'wek','hp_wek_sst_week_cross','hp_wek_sst_week_fixed','hp_wek_crlg_week')
    
    
    wek(find(abs(rm.*wek)>wek_th))=nan;
    hp_wek_crlg_week(find(abs(rm.*hp_wek_crlg_week)>wek_th))=nan;
%     hp21_wek_crlg_week(find(abs(rm.*hp21_wek_crlg_week)>wek_th))=nan;
    
    if exist('hp_wek_sst_week_cross')
        hp_wek_sst_week_cross(find(abs(rm.*hp_wek_sst_week_cross)>sst_th))=nan;
        hp_wek_sst_week_fixed(find(abs(rm.*hp_wek_sst_week_fixed)>sst_th))=nan;
    end

    eval(['save -append ' [rpath num2str(jdays(m))] ' lat lon *_week hp* wek'])
end
