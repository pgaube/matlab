...
for m=58:length(jdays)
    ...
    load([spath num2str(jdays(m))],'sm_u_week','sm_v_week','hp_wek_sst_week_dtdn','hp_wek_crlg_week','hp66_crlstr','hp66_wspd')
...
%     
    ...
    hp_wek_crlg_week(tt)=nan;tc(tt)=2;
    
    
    if ~exist('hp_wek_sst_week_dtdn')
        hp_wek_sst_week_dtdn=nan*wek;
    end
    ts=ones(size(hp66_crlstr));
    ts(isnan(hp_wek_sst_week_dtdn))=nan;
    tt=find(abs(rm.*hp_wek_sst_week_dtdn)>sst_th);
    
    hp_wek_sst_week_dtdn(tt)=nan;ts(tt)=2;
    ...
    title(['QuikSCAT ',num2str(100*(length(find(tw==2))./length(find(tw==1)))),' removed'])
    subplot(212)
    ...
    subplot(212)
    pmap(lon,lat,tc);caxis([1 2]);colormap(bwr)
    xlabel(num2str(jdays(m)))
    ...
    title(['SST ',num2str(100*(length(find(ts==2))./length(find(ts==1)))),' removed'])
    subplot(212)
    pmap(lon,lat,ts);caxis([1 2]);colormap(bwr)
    xlabel(num2str(jdays(m)))
