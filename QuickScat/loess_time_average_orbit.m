...
%
% Initialize the matrix to store values for loess reg
%
    ...
    [tmp_swspd,     tmp_sdwspddx,  tmp_sdwspddy, ...
     ...
     tmp_sdiv,      tmp_scrl ...
     tmp_sdivstr,   tmp_scrlstr] = deal(single(nan(length(r),length(c),length(iorbit),1)));
     ...
     Wdivstr,   Wcrlstr,   Wdstrmds,  Wdstrmdn] = deal(single(nan(ny,nx)));     
		...
		fprintf('\r|  time level %01u of %02u | \r',k,length(iorbit))
     	...
     	dvstrdy = (dvstrdy(SY(r),SX(c)));
     		...
     		%{
     		wspd(isnan(wspd))		=1e35;
     		dwspddx(isnan(dwspddx))	=1e35;
     		dwspddy(isnan(dwspddy))	=1e35;
     		strm(isnan(strm))		=1e35;
     	dstrmdx(isnan(dstrmdx))	=1e35;
     	dstrmdy(isnan(dstrmdy))	=1e35;
     	u(isnan(u))				=1e35;
     	dudx(isnan(dudx))		=1e35;
     	dudy(isnan(dudy))		=1e35;
     	v(isnan(v))				=1e35;
     	dvdx(isnan(dvdx))		=1e35;
     	dvdy(isnan(dvdy))		=1e35;
     	ustr(isnan(ustr))		=1e35;
     	dustrdx(isnan(dustrdx))	=1e35;
     	dustrdy(isnan(dustrdy))	=1e35;
     	vstr(isnan(vstr))		=1e35;
     	dvstrdx(isnan(dvstrdx))	=1e35;
     	dvstrdy(isnan(dvstrdy))	=1e35;
     	%}
