...
pmask=nan*full_sst;
pmask((full_sst)>min(ran)-1)=1; %pmask((full_sst)>min(ran)+2)=1; 
psst=sst.*pmask;
buff_sst=buffnan_rad(sst,2);
...
if per>=.15 & length(zt)<=1000 & length(nav_off)<5e3
		%fplot(buff_sst,mask,ran(1),ran(2),'fdcolor',[AUTO_IMAGE_DIR 'bdsc_'],pd,tmp_time,'sst')
		%fplot(tmp_sci,mask,ran(1),ran(2),'fdcolor',[L0_IMAGE_DIR 'bdsc_'],pd,tmp_time,'sst')
		%fplot(psst,mask,ran(1),ran(2)+1,'fdcolor',[AUTO_IMAGE_DIR 'bdsc_'],pd,tmp_time,'sst')
		fplot(psst,mask,ran(1)+1,ran(2)+cool_eve,'fdcolor',[L0_IMAGE_DIR 'bdsc_'],pd,tmp_time,'sst')
	else
...
for m=1:length(tmp)
	...
	OBS(:,:,m)=tmp_sst;
	ASC(:,:,m)=tmp_sci;
	if length(nav_off)<300 & length(zt)<=.2*(length(mlat(:,1))*length(mlon(1,:)))
...
fplot(nanmean(PST,3),mask,ran(1),ran(2),'fdcolor',[COMP_IMG_DIR 'bdsc_'],pd,tmp_time,'sst')
fplot(nanmean(PST,3),mask,ran2(1),ran2(2),'fdcolor',[COMP_IMG_DIR 'bdsc_'],pd,tmp_time,'sst')
%fplot(max(PST,[],3),mask,ran(1),ran(2),'fdcolor',[COMP_IMG_DIR 'bdsc_'],pd,tmp_time,'sst')
%fplot(max(PST,[],3),mask,ran2(1),ran2(2),'fdcolor',[COMP_IMG_DIR 'bdsc_'],pd,tmp_time,'sst')
