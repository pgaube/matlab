...
for m=1:length(sq(:,1))
	...
	m_proj('Miller Cylindrical','lon',[sq(m,3) sq(m,4)],'lat',[sq(m,2) sq(m,1)]);
	m_gshhs_f('save','gumby');

	%now make land mask for data proccessing
	m_pcolor([sq(m,3):sq(m,4)],[sq(m,2):sq(m,1)],nan(length([sq(m,2):sq(m,1)]),length([sq(m,3):sq(m,4)])));axis image
	shading flat
	...
	eval(['print -dpng -r300 ', HOME_DIR, '/land_masks/',labs(m,:),'_load_mask.png'])
	eval(['crop(',char(39), HOME_DIR, '/land_masks/',labs(m,:),'_load_mask.png',char(39),')'])
	%fl=input('edit mask')
	eval(['mask=flipud(nanmean(imread(',char(39), HOME_DIR, '/land_masks/',labs(m,:),'_load_mask.png',char(39),'),3));'])
	mask(mask==255)=1;
	mask(mask==0)=nan;
	mask(:,1:2)=[];
	mask(1:2,:)=[];
	...
	imask=nan*mask;
	imask(isnan(mask))=1;
	mlat=linspace(sq(m,2),sq(m,1),length(mask(:,1)));
	mlon=linspace(sq(m,3),sq(m,4),length(mask(1,:)));
	[mlon,mlat]=meshgrid(mlon,mlat);
	
	[tout,lonout,latout]=get_topo([sq(m,3):sq(m,4)],[sq(m,2):sq(m,1)]);
	topo=griddata(-180-(180-lonout),latout,tout,mlon,mlat,'linear');
	rname=labs(m,:)
	save([labs(m,:),'_sf_mask'],'mlat','mlon','mask','imask','topo','rname')
	...
	q=ones(256,1);
	q(256)=0;     
    imwrite(irgb,pgray,[ HOME_DIR, '/land_masks/',labs(m,:),'_mask.png'],'Transparency',q)	
	...
	imwrite(irgb,topomap1,[ HOME_DIR, '/land_masks/',labs(m,:),'_topo.png']);
	
	minc=-3500;
	maxc=2750;
	irgb = max(min(round((idata-minc)*cstep+1),nrgb),1).*flipud(mask);
	imwrite(irgb,topomap1,[ HOME_DIR, '/land_masks/',labs(m,:),'_bath.png']);
	...
	eval(['print -dpng -r300 ',  HOME_DIR, '/land_masks/',labs(m,:),'_cont.png'])
	eval(['crop(',char(39), HOME_DIR, '/land_masks/',labs(m,:),'_cont.png',char(39),')'])
	conts=imread([ HOME_DIR, '/land_masks/',labs(m,:),'_cont.png']);
	%{
	conts(:,1:2)=[];
	conts(1:2,:)=[];
	conts(:,end-2:end)=[];
	...
	imwrite(mconts,[ HOME_DIR, '/land_masks/',labs(m,:),'_cont.png']);
end	