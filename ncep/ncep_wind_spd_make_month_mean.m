...
for i = years
    ...
    for p = 1:12
        if p==1
           ...
           mtmpc = dtmpc(:,:,1:days(1));
        else
           mtmp = dtmp(:,:,days(p-1)+1:days(p));
           mtmps = dtmps(:,:,days(p-1)+1:days(p));
           mtmpc = dtmpc(:,:,days(p-1)+1:days(p));
        end
        m(:,:,p) = mean(mtmp,3);
        ms(:,:,p) = mean(mtmps,3);
        mc(:,:,p) = mean(mtmpc,3);
    end
    clear dtmp dtmps dtmpc
    eval([fname2 '= m;']);
    eval([fname2s '= ms;']);
    eval([fname2c '= mc;']);
...
clear i j years days p path fname fname2 fname2s fname2c wind_spd wind_spd_s wind_spd_c g k fname_u fname_v
