...
for m=1:length(jdays)
	...
	load(r_file,'nR')
	load(q_file,'hp66_crl','hp_wek_sst_week_dtdn')
   	mask=ls_mask;
   	%mask=ls_half_mask;
	...
	hp66_crl=hp66_crl.*mask(rs,cs);
	sd=pstd(hp66_crlg);
	y=hp66_crlg+(.1*sd*nR).*mask;
	yy=hp66_crlg+(sd*nR).*mask;
   	
   	tmp_var(:,:,m)=single(hp66_crlg(r,c));
   	tmp_var_na(:,:,m)=single(y(r,c));
   	tmp_var_ha(:,:,m)=single(yy(r,c));
