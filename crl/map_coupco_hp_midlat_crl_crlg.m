%
%loads SeaWiFS CHL data
clear all
close all


jdays=[2452459:7:2454489];%[2451556:7:2454797];
lj=length(jdays);
%lj=58

asave_path='/matlab/data/eddy/V4/mat/';
asave_head='AVISO_25_W_';
qsave_path='/matlab/data/QuickScat/mat/';
qsave_head='QSCAT_30_25km_';

load([asave_path asave_head num2str(jdays(1))],'lon','lat')
slat=lat;
slon=lon;
load([qsave_path qsave_head num2str(jdays(1))],'lon','lat')
[rs,cs]=imap(min(lat(:)),max(lat(:)),1,1,slat,slon);

[year,month,day]=jd2jdate(jdays(1:lj));
%

CRLG=single(nan(560,1440,lj));
CRL=CRLG;


for m=1:lj
	fprintf('\r    loading %03u of %03u\r',m,lj)
	load([qsave_path qsave_head num2str(jdays(m))],'mask','bp26_crl')
	qs_mask=mask;
	load([asave_path asave_head num2str(jdays(m))],'hp66_crlg','ls_mask')
	mask=ls_mask;
	CRL(:,:,m)=single(bp26_crl.*qs_mask.*mask(rs,:));
	CRLG(:,:,m)=single(hp66_crlg(rs,:).*qs_mask.*mask(rs,:));
	clear bp26* hp66* *mask*
end

save -v7.3 all_ls_crls lat lon CRL CRLG year month day jdays
return
%}
load all_ls_crls
beta_n=single(nan(560,1440));

dist=16 %in native data res

for m=1:560
	fprintf('\r m = %3u',m)
	for n=1:1440
		if m<dist+1
			ros=1:m+dist;
		elseif m>560-dist-1
			ros=m-dist:560;
		else
			ros=m-dist:m+dist;
		end
		if n<dist+1
			col=1:n+dist;
		elseif n>1440-dist-1
			col=n-dist:1440;
		else
			col=n-dist:n+dist;
		end
		x=squeeze(CRLG(ros,col,:));
		y=squeeze(CRL(ros,col,:));
		x=x(:);
		y=y(:);
		tbins=-2e-5:.1e-5:2e-5;
		beta_n(m,n)=ols_pca(-x,y);
	end
end	
sm_coupco_n=smoothn(beta_n,20);


save coupco_map_hp_crls beta_* lat lon sm_coupco_*
