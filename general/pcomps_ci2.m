function pcomps_ci2(comp_dat,cont,cax,mincont,conti,maxcont,tit)

load chelle.pal
load bwr.pal

imag=interp2(comp_dat.mean);
stdd=interp2(smoothn(comp_dat.std,5));
nn=comp_dat.n_eddies;
ci=abs(stdd.*tinv((.05)/2,nn-1)./sqrt(nn));
cont=interp2(cont);
...
h=pcolor(x,y,imag);shading interp;colormap(chelle);
set(h,'facecolor', 'interp', 'FaceAlpha',.25)
caxis(cax)
hold on
