function eddies=track_eddies_MDT(lon,lat,jdays,ssh,mdt,dx,dt_step)
...
max_pix=200*(.25/dx);      %maximum number of pixels within eddy
max_dist=100;                %maximum distance an eddy can propagate in one dt to be consider part of pervious observed eddy
max_dist_pnt=300;
...
%             clf
%             subplot(211)
%             contour(lon(1,:),lat(:,1),mdt(:,:,it).*nmask,[ll ll],'k');
%             hold on
%             if edd_pnt>1
%                 plot(x,y,'k*')
%             end
...
%             
%             subplot(212)
%             contour(lon(1,:),lat(:,1),ssh(:,:,it).*nmask,[ll ll],'k');
%             hold on
%             if edd_pnt>1
%                 plot(x,y,'k*')
%             end
...
%                                 end
    ...
    clf
%     subplot(211)
    pmap(lon,lat,ssh(:,:,it).*amask);caxis([-40 40])
    hold on
...
%     pmap(lon,lat,mdt(:,:,it));
%     hold on
% %     m_contour(lon,lat,mdt(:,:,it),[1:5:500],'k')
%     %     m_contour(lon,lat,mdt(:,:,it),[-500:1:-1],'k--')
%     for m=1:length(ia)
%         m_plot(x(ia(m)),y(ia(m)),'k*')
%     end
%     
                ...
                if any(itime2)
                    ...
                    if any(ii)
                        ...
                        if dist_y(ii)<=max_dist & ...
                                ...
                                dist>0  & ...
                                status(itime2(ii))==1 %if pass, found an untaken eddy to attach it to
