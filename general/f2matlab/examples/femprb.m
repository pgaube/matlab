...
if ~isempty(inputname(1)), assignin('caller','FUntemp',i); evalin('caller',[inputname(1),'=FUntemp;']); end
return;
end
...
if( strcmpi(code,'Q4') ) ;
order_codeResult = 4;
elseif( strcmpi(code,'Q8') ) ;
order_codeResult = 8;
elseif( strcmpi(code,'Q9') ) ;
order_codeResult = 9;
elseif( strcmpi(code,'Q12') ) ;
order_codeResult = 12;
elseif( strcmpi(code,'Q16') ) ;
order_codeResult = 16;
elseif( strcmpi(code,'QL') ) ;
order_codeResult = 6;
elseif( strcmpi(code,'T3') ) ;
order_codeResult = 3;
elseif( strcmpi(code,'T6') ) ;
order_codeResult = 6;
elseif( strcmpi(code,'T10') ) ;
order_codeResult = 10;
else;
order_codeResult = 0;
end;
if ~isempty(inputname(1)), assignin('caller','FUntemp',code); evalin('caller',[inputname(1),'=FUntemp;']); end
return;
end
...
if isempty(i), i=0; end;
%
[ code, n, r, s, t, dtdr, dtds ]=shapemlv( code, n, r, s, t, dtdr, dtds );
...
fprintf(1,'%s \n','-----------------------------------------------------------------------');
fprintf(1,'%s ','w=');fprintf(1,'%0.15g \n',w);
fprintf(1,'%s ','lda=');fprintf(1,'%0.15g \n',lda);
fprintf(1,'%s ','n=');fprintf(1,'%0.15g \n',n);
fprintf(1,'%s ','ipivot=');fprintf(1,'%0.15g \n',ipivot);
fprintf(1,'%s ','info=');fprintf(1,'%0.15g \n',info);
if( info ~= 0 ) ;
writef(1,['%s \n'], ' ');
writef(1,['%s \n'], 'MAP - Fatal error!');
...
r_shape=zeros(r_shape);r_shape(1:numel(r))=r;r=r_shape;
s_shape=zeros(s_shape);s_shape(1:numel(s))=s;s=s_shape;
return;
end
...
rexp_shape=zeros(rexp_shape);rexp_shape(1:numel(rexp))=rexp;rexp=rexp_shape;
sexp_shape=zeros(sexp_shape);sexp_shape(1:numel(sexp))=sexp;sexp=sexp_shape;
return;
end
...
function [code, n, r, s, t, dtdr, dtds]=shapemlv( code, n, r, s, t, dtdr, dtds ,varargin);
%
%*******************************************************************************
%
...
writef(1,['%s \n'], ' ');
for i = 1: n;
[ code, n, r(i), s(i), t, dtdr, dtds ]=shapemlv( code, n, r(i), s(i), t, dtdr, dtds );
