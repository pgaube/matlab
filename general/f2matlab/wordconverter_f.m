...
switch funstrwords{i}{j}
 ...
 case {'stop'}
  outflag(1)=1;
  funstr{i}=[funstr{i}(1:funstrwords_b{i}(j)-1),['error(''stop encountered in original fortran code'');']];
 case {'common'}
  outflag(1)=1;
...
%%%    end
    if ~isempty(groups{ii})
     if strncmp(fliplr(deblank(groups{ii})),'''',1)
      groupsStr=[groupsStr,groups{ii},'=fscanf(',thisfid,',''%s',sp,'',temp,''',size(',groups{ii},'));'];
     else
      groupsStr=[groupsStr,groups{ii},'=fscanf(',thisfid,',''%g',sp,'',temp,''',size(',groups{ii},'));'];
     end   
    else
...
%%%   funstr=replaceword_f(i,j,funstr,funstrwords,funstrwords_b,funstrwords_e,repstr);
 ...
 case {'inquire'}
  ...
  if howmany==2
   ...
   warning('inquire -- f2matlab can only handle inquire(file=,exists=) at the moment')
  end
 case {'isha'}
 case {'ishc'}
 case {'ishft'}
...
%%%  end
 ...
 case {'maxexponent'}
 case {'maxloc'}
  disp('''maxloc'' not implemented at this time, help max to do this in Matlab.')
 case {'maxval'}
  repstr='max';outflag(1)=1;
  funstr=replaceword_f(i,j,funstr,funstrwords,funstrwords_b,funstrwords_e,repstr);  
 case {'merge'}
...
%%%  funstr{i}=[funstr{i}(1:(funstrwords_b{i}(j)-1)),'datestr(rem(now,1))',funstr{i}(funstrwords_e{i}(j)+1:end)];
 ...
 case {'ubound'}
  ...
  funstr=replaceword_f(i,j,funstr,funstrwords,funstrwords_b,funstrwords_e,repstr);  
 case {'unpack'}
 case {'val','loc','ref'} %we just have to get rid of the .val() type constructs
  outflag(1)=1;
  goon=0;
...
%%%  outflag(1)=1;
%%%  [howmany,subscripts,centercomma,parens]=hassubscript_f(i,j,funstr,funstrnumbers,funstrnumbers_b,funstrnumbers_e,funstrwords,funstrwords_b,funstrwords_e,funwords);
%%%  funstr{i}=[funstr{i}(1:funstrwords_b{i}(j-1)-1),strtrim(subscripts{1}),'=strtrim(',...
...
%%%   end % if eqLoc~=-1
%%%  end % if howmany>0
%%% case {'countdtablerows','get_data_period'}
%%%  outflag(1)=1;
%%%  [howmany,subscripts,centercomma,parens]=hassubscript_f(i,j,funstr,funstrnumbers,funstrnumbers_b,funstrnumbers_e,funstrwords,funstrwords_b,funstrwords_e,funwords);
%%%  if howmany>0
%%%   eqLoc=findNext(funstrwords_b{i}(j),'=',funstr{i},-1);
...
%%%  funstr=replaceword_f(i,j,funstr,funstrwords,funstrwords_b,funstrwords_e,repstr);  
%%% case {'loadconstraintvectorsparse'}
%%%  outflag(1)=1;
%%%  [howmany,subscripts,centercomma,parens]=hassubscript_f(i,j,funstr,funstrnumbers,funstrnumbers_b,funstrnumbers_e,funstrwords,funstrwords_b,funstrwords_e,funwords);
%%%  if howmany==6 && j>1
...
%%%  end
%%% case {'data_read_character_anysize','data_read'}
%%%  outflag(1)=1;
%%%  [howmany,subscripts,centercomma,parens]=hassubscript_f(i,j,funstr,funstrnumbers,funstrnumbers_b,funstrnumbers_e,funstrwords,funstrwords_b,funstrwords_e,funwords);
%%%  if howmany>0 & j>1
%%%   eqLoc=findNext(funstrwords_b{i}(j),'=',funstr{i},-1);
...
%%%   funstr{i+1}=[funstrwords{i}{argWordOut},'=error_no_error;'];
%%%   %funstr(i:i+1),kb
%%%  end  
...
%%%   funstr{i}=[funstr{i}(1:funstrwords_b{i}(argWordOut)-1),'try, ',funstrwords{i}{argWordOut}...
%%%              ,'=evalin(''base'',',...
...
%%%   funstr{i+1}=[funstrwords{i}{argWordOut},'=0;'];
%%%  end
%%% case {'gmomxcopyptrtoreal8','gmomxcopyptrtoreal4'}
%%%  outflag(1)=1;
%%%  [howmany,subscripts,centercomma,parens]=hassubscript_f(i,j,funstr,funstrnumbers,funstrnumbers_b,funstrnumbers_e,funstrwords,funstrwords_b,funstrwords_e,funwords);
%%%  if howmany>0 && j>1 && strcmp(funstrwords{i}{j-1},'call')
%%%   funstr{i}=[funstr{i}(1:funstrwords_b{i}(j-1)-1),strtrim(subscripts{2}),'=',...
...
if any(in=='(') % we have a recursive definition
 ...
 if ~inastring_f(in,temp) & ~inastring2_f(in,temp)
  ...
  %'rrrrrrrrrrr',groups,groupsStr,kb
  return
 end
end
