function out=buildTypeDefLine(typeDefs,i,var_words,want_row)
 ...
 if ~isempty(temp)
  ...
  if ~isempty(typeDefs{i,2}{j,11})
   ...
   elseif howmany==1
    if ~any(strcmp(localVar{temp6,3},var_words))
     if want_row
      outZ=[localVar{temp6,3},'(ones(1,',localVar{temp6,5}{1},'))'];
     else
      outZ=[localVar{temp6,3},'(ones(',localVar{temp6,5}{1},',1))'];
     end
    elseif strcmp(localVar{temp6,3},'character')% && length(localVar{temp6,5})>0
     outZ=['{cell(1,',char(localVar{temp6,5}{1}),')}'];
     %outZ=[localVar{j,1},'=cell(1,',localVar{temp6,2},');'];
    else
     if want_row
      outZ=['zeros(1,',localVar{temp6,5}{1},')'];
     else
      outZ=['zeros(',localVar{temp6,5}{1},',1)'];
     end
    end
   else
    if ~any(strcmp(localVar{temp6,3},var_words))
    ...
    for k=1:howmany
     if k~=howmany
      subscripts2=[subscripts2,localVar{temp6,5}{k},','];
     else
      subscripts2=[subscripts2,localVar{temp6,5}{k},')'];
     end
    end
