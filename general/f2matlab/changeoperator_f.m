 ...
 switch operator
  case {'==','~='}
   if funstr{i}(leftarg_b)=='''' | funstr{i}(rightarg_b)=='''' | goon
    ...
    if all(isspace(funstr{i}(rightarg_b+1:rightarg_e-1)))
     funstr{i}=[funstr{i}(1:(leftarg_b-1)),temp6,'isempty(strtrim(',...
                funstr{i}(leftarg_b:leftarg_e),'))',funstr{i}((rightarg_e+1):end)];
    else
     funstr{i}=[funstr{i}(1:(leftarg_b-1)),temp6,'strcmpi(',funstr{i}(leftarg_b:leftarg_e),',',funstr{i}(rightarg_b:rightarg_e),')',funstr{i}((rightarg_e+1):end)];
    end
    changedflag=1;
   end
  case {'<','>','<=','>='}
   % fix these for character arrays to call strrel
   if funstr{i}(loc+1)~='=' && length(funstrwords{i})>0 && any(strcmp(funstrwords{i}{1},{'ig','elseif'}))
    if funstr{i}(leftarg_b)=='''' | funstr{i}(rightarg_b)=='''' | goon
