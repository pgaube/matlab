function eddies=track_eddies_CI(lon,lat,jdays,ssh,ci_data,CI,dx,dt_step)
...
%ssh[m,n,p]         SSH in cm
%ci_data[m,n,p]     data from which closed contours are identified
...
min_amp=0;                  %minimum amplitude of eddy
min_k=1;                    %minmim lifetime of eddy
npt=1;                      %number of points on each side of eddy centroid used to compute scale
min_pix=1*(.25/dx) ;        %min number of pixels within eddy
max_pix=500*(.25/dx);       %maximum number of pixels within eddy
...
for it=1:tm
    ...
    for ll=1:length(CI) %loop through ci_data filed one CI at a time
        C=contourc(lon(1,:),lat(:,1),ci_data(:,:,it).*nmask,[CI(ll) CI(ll)]);
    ...
    for m=1:length(itime)
        ...
        if any(ii)
            ...
            if dist_y(ii)<=max_dist & ...
                    ...
                    dist>0  & ...
                    status(itime1(ii))==1%if pass, found an untaken eddy to attach it to
                            ...
                            if any(itime3)
                                ...
                                if any(ii)
                                    ...
                                    if dist_y(ii)<=max_dist & ...
                                            ...
                                            dist>0  & ...
                                            status(itime3(ii))==1%if pass, found an untaken eddy to attach it to
