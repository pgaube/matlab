function [compa,compc] = mcomps_id(ids);
%function [compa,compc] = mcomps_id(ids);
%
% Makes composest of the samples for the WHOLE LIFE OF AN EDDY
%
% INPUT:
% 
...
load /matlab/data/eddy/V4/global_tracks_V4_12_weeks nneg id track_jday
...
load([OUT_PATH OUT_HEAD num2str(startjd+14)],'nrssh_sample');
dat=nrssh_sample;
[M]=length(r_comp);
...
for m=1:lj
    ...
    if any(ii)
        ...
        for pp=1:length(ii)
            ...
            crl_obs=nrbp26_crl_sample(r_comp,c_comp,ii(pp));
...
compa.crl_median  			= nanmedian(crl_compa,3);
compa.crl_N 				= crl_Na-1;
compa.crl_n_max_sample		= max(crl_Na(:));
compa.crl_per_cov			= 100*(crl_Na./max(crl_Na(:)));

compa.crlg_median  			= nanmedian(crlg_compa,3);
compa.crlg_N 				= crlg_Na-1;
compa.crlg_n_max_sample		= max(crlg_Na(:));
compa.crlg_per_cov			= 100*(crlg_Na./max(crlg_Na(:)));
...
compc.crl_median  			= nanmedian(crl_compc,3);
compc.crl_N 				= crl_Nc-1;
compc.crl_n_max_sample		= max(crl_Nc(:));
compc.crl_per_cov			= 100*(crl_Nc./max(crl_Nc(:)));

compc.crlg_median  			= nanmedian(crlg_compc,3);
compc.crlg_N 				= crlg_Nc-1;
compc.crlg_n_max_sample		= max(crlg_Nc(:));
compc.crlg_per_cov			= 100*(crlg_Nc./max(crlg_Nc(:)));
