...
[tmp_var1_a]=single(nan(length(id).*length(~isnan(mask))*5,1));
...
for m=1:length(jdays)
    ...
    for pp=1:length(ii)
            ...
            tmp_var1_a(sta:sta-1+length(obs1(:)),1)=single(obs1(:));
            tmp_var2_a(sta:sta-1+length(obs1(:)),1)=single(obs2(:));
            sta=sta+length(obs1(:));
            else
            tmp_var1_c(stc:stc-1+length(obs1(:)),1)=single(obs1(:));
            tmp_var2_c(stc:stc-1+length(obs1(:)),1)=single(obs2(:));
            stc=stc+length(obs1(:));
            end
...
eval(['save ' var1 '_' var2 '_' EDDY_NAME '_coupcoef_eddy tmp_var* var1 var2'])
fprintf('\n')