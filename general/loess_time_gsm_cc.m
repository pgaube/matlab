...
%startjd=2450884
%endjd=2454832
path_in='/matlab/data/gsm/mat/GSM_9_D_';
path_out='/matlab/data/gsm/mat/GSM_9_21_';
dt=30
METHOD='none'
...
for pp=1:M*N
    ...
    for m=1:length(jdays)
       ...
       if exist(fname)
        	load(fname, 'gchl_day','gcar_day')
        	data_in=gchl_day(r,c)./gcar_day(r,c);
        else
        	fprintf('\r missing file \n')
        	fname
        	data_in = nan*gcar_day(r,c);
        end
        subset_lt_chl(rs,cs,m)=single(data_in);
    end
    ...
    for mm=1:length(rs)
        for nn=1:length(cs)
        	...
        	case {'hp_flag'}
        		...
        		tmp=interm.*fbad;
        		%remove longterm mean
        		lp=smooth1d_loess(tmp,interp_jdays',200,interp_jdays');
        		beta(rr,cc,:)=tmp;
        		beta_hp(rr,cc,:)=tmp-lp;
        	case {'flag'}
            	[interm,flag]=smooth1d_loess(squeeze(subset_lt_chl(rrs,ccs,:)),jdays',dt,interp_jdays');
