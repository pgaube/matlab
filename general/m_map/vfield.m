...
%if ~isstr(a), r = speed*a;					%variable arrow head size
%else          r = repmat(str2num(a),size(speed)); en


%if ~isstr(a), r = repmat(.005,size(speed));			%global arrow head size
%else          r = repmat(.005,size(speed)); end

if ~isstr(a), r = repmat(.0015,size(speed));			%fixed arrow head size
else          r = repmat(.0015,size(speed)); end
