...
bobcallbad = strcat(urlbase,'?get=griddata&dataset=',strcat(satid,param),...
		   '&timeperiod=',duration,...
		   '&centeredTime=');
string=urlread(bobcallbad);

% sift through the crap for ISO dates
stind = regexp(string,'<nobr>','start');
endind = regexp(string,'</nobr>','end');
icnt = 1;
for i = 1:length(stind),
    str2 = string(stind(i):endind(i));
    stind2 = regexp(str2,'\d\d\d\d\-\d\d-\d\d','start');
    endind2 =  regexp(str2,'\d\d\d\d\-\d\d-\d\d','end');
    if ~isempty(stind2)
      for j = 1:length(stind2),
        alldates(icnt,1:19) = str2(stind2(j):endind2(j)+9);
        icnt = icnt + 1;
      end
    end
end
