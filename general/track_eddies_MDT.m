function eddies=track_eddies_MDT(lon,lat,jdays,mdt,ssh,dx,dt_step)
...
%mdt[m,n,p]         SSH anomaly in cm
%mdt[m,n,p]         Absolute SSH in cm
...
%                   defining the eddy.
%eddies.mask        Mask the same size as mdt with pixels located within an
...
max_dist=75;               %maximum distance an eddy can propagate in one dt to be consider part of pervious observed eddy
max_dist_pnt=500;
thres_y_dist=75;           %Set the max y-dist eddies can be to west of orginal eddy
...
for it=1:tm
    ...
    if ~isempty(find(~isnan(mdt(:,:,it))))
        nmask=ones(size(mask(:,:,it)));
        eid_mask=nan(size(nmask));
        u=-9.81./ffff.*dfdy(.01*ssh(:,:,it),dx);
        v=9.81./ffff.*dfdx(lat,.01*ssh(:,:,it),dx);
        spd=9.81*sqrt(v.^2+u.^2); %cm/s
        crl=dfdx(lat,v,dx)-dfdy(u,dx);

        %first identify anticyclones
        for ll=-500:500 %loop through SSH filed 1 cm at a time
            C=contourc(lon(1,:),lat(:,1),mdt(:,:,it).*nmask,[ll ll]);
                ...
                while ed<length(C(1,:))
                    if C(1,st)==C(1,ed) & C(2,st)==C(2,ed)
                        ...
                        if length(find(inmask==1))>min_pix & length(find(inmask==1))<max_pix
                            ...
                            if pmean(sign(ffff.*ttm))*pmean(rrc)<0
                                ii=find(mrc==max(mrc(:)));
                                ...
                                if length(ii)==1
                                    ...
                                    if max(distpnt(:))<=max_dist_pnt;
                                        ...
                                        x(edd_pnt)=lon(ii);
                                        cyc(edd_pnt)=1;
                                        ro(edd_pnt)=abs(pmean(rrc(:)))/pmean(ffff.*ttm);
        ...
        for ll=500:-1:-500 %loop through SSH filed 1 cm at a time
            C=contourc(lon(1,:),lat(:,1),mdt(:,:,it).*nmask,[ll ll]);
            %find closed contours
            if length(C(1,:))>5
                tmp=nan*C;
...
%         clf
%         %     subplot(211)
%         pmap(lon,lat,mdt(:,:,it).*amask);caxis([-40 40])
%         hold on
%         ia=find(cyc==1 & track_jday==jdays(it));
%         ic=find(cyc==-1 & track_jday==jdays(it));
%         for m=1:length(ia)
%             m_plot(x(ia(m)),y(ia(m)),'k*')
%         end
%         
%         for m=1:length(ic)
%             m_plot(x(ic(m)),y(ic(m)),'k.')
%         end
        pause(1)
    end
end
    ...
    for m=1:length(itime)
        ...
        if any(ii)
            ...
            if dist_y(ii)<=max_dist & ...
                    dist_x(ii)<=max_dist & ...
                    dist_x(ii)>=-thres_y_dist & ...
                    dist>0  & ...
                    status(itime1(ii))==1 & ...
                    ro(itime1(ii))>=0.25*ro(itime(m)) & ...
                    ro(itime1(ii))<=2.5*ro(itime(m))%if pass, found an untaken eddy to attach it to
                ...
                if any(itime2)
                    ...
                    if any(ii)
                        ...
                        if dist_y(ii)<=max_dist & ...
                                dist_x(ii)<=max_dist & ...
                                dist_x(ii)>=-2*thres_y_dist & ...
                                dist>0  & ...
                                status(itime2(ii))==1 & ...
                                ro(itime2(ii))>=0.25*ro(itime(m)) & ...
                                ro(itime2(ii))<=2.5*ro(itime(m))%if pass, found an untaken eddy to attach it to
                            ...
                            if any(itime3)
                                ...
                                if any(ii)
                                    ...
                                    if dist_y(ii)<=max_dist & ...
                                            dist_x(ii)<=max_dist & ...
                                            dist_x(ii)>=-3*thres_y_dist & ...
                                            dist>0  & ...
                                            status(itime3(ii))==1 & ...
                                            ro(itime3(ii))>=0.25*ro(itime(m)) & ...
                                            ro(itime3(ii))<=2.5*ro(itime(m))%if pass, found an untaken eddy to attach it to
...
for m=1:length(uid)
    ...
    if any(ibad)
        ...
        eddies.ro(ptn+1:ptn+length(new_jday))=interp1(tmp_track_jday,tmp_ro,new_jday,'linear');
