...
path3='/Users/gaube/Documents/NSU/APPTIV_PB/data/daily/Seachl/';    %set this path to the directory                                                               %containg the SeaWiFS data   
g='.gz';
        ...
        for k=1:numdays;
            ...
            if exist(fcheck);
                ...
                intmat(:,:,2)=sst2;
                 ...
                 eval(['!bzip2 -d ' path3 filename3 '.bz2']);
                 data = eval(['hdfread(' char(39) path3 filename3 char(39) ',' char(39) 'l3m_data' char(39) ',' char(39) 'Index' char(39) ',' ' ' '{[1.0 1.0 ],[1.0 1.0 ],[2048.0 4096.0 ]}' ')']);
				 ...
				 data = flipdim(data,1);
				 %sCHL = biggrid(data);
				 sCHL = grid2d(data,lat,lon,LAT,LON,STEP,STEP,'ave'); 
				 x = find(isnan(sCHL));
				 sstm(x) = nan;
				 eval(['!bzip2 ' path3 filename3]);
				 %clear sCHL data
				 [tCLD,n]=biggrid(sstm);
