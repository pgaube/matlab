function eddies=track_eddies_PG2(lon,lat,jdays,ssh,dx,dt_step)
...
nd=round(2*(.25/dx));       %number of pixels around eddy SSH extemum that geometric criteria are checked
                        ...
                        if min(rrc(:))>eps
                            ...
                            if length(find(rrc==max(rrc(:))))==1
                                ...
                                if rrc(ii)-cont_ssh>min_amp
                                    %check geometic constrains
                                    [r,c]=imap(lat(ii)-dx/2,lat(ii)+dx/2,lon(ii)-dx/2,lon(ii)+dx/2,lat,lon);
                                    if sign(u(r,c-nd))~=sign(u(r,c+nd)) & sign(v(r-nd,c))~=sign(v(r+nd,c))
