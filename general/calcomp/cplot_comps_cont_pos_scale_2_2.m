function cplot_comps_cont_pos_scale_2_2(data1,data2,minc,maxc,dz,mincc,maxcc,title,out_file)

% Set up paths, adjust for your machine
%

func_path = '/matlab/calcomp/comps_cont_pos_scale_2_2.f'; 
...
eval(['!sed ' char(39) 's/parameter ( mincc=-5, maxcc=-5)/parameter ( mincc=' num2str(mincc) ', maxcc=' num2str(dz) ')/' char(39) ' tmp33.f > tmp44.f']);
eval(['!sed ' char(39) 's/test/' title '/' char(39) ' tmp44.f > tmp55.f']);
eval(['!sed ' char(39) 's/969/' num2str(length(title)) '/' char(39) ' tmp55.f > tmp.f']);
