...
load_path = '/Volumes/matlab/matlab/global/modis_chl_4km/'
load([load_path,'CHL_4_W_2453278'])
lat=single(lat);
lon=single(lon);
...
for pp=1:M*N
    ...
    for m=1:length(mid_week_jdays)
        ...
        load(fname, 'chl_week')
        subset_chl(rs,cs,m)=single(chl_week(r,c));
    end
        ...
        for nn=1:length(cs)
            ...
            [ny,beta(rr,cc,:),S(rr,cc),theta] = ...
                    harm_reg(mid_week_jdays,...
                             squeeze(subset_chl(rrs,ccs,:)),...
                             2,...
                             2*pi/365.25);
            NumCov(rr,cc) = single(length(find(~isnan(squeeze(subset_chl(rrs,ccs,:))))));
        end
    end  
    ...
    fprintf('\n')
    save([load_path,'filtered_raw_chl_stuff'],'beta','S','NumCov','lat','lon',...
         'r*','c*','load_path','mid_week_jdays','jdays','subset_chl')
...
for m=223:length(mid_week_jdays)
    fprintf('\r     io file %03u of %03u \r',m,length(mid_week_jdays))
    fname = [load_path 'CHL_4_W_' num2str(mid_week_jdays(m)) '.mat'];
    load(fname,'chl_week')
    raw_SS = (beta(:,:,1)...
