...
save dCC_raw_profiles tmp_* t_* eddy_* s_* tpres ppres lap nneg lat lon
%}

load dCC_raw_profiles
warning('off','all')
repeats=3;
...
save dCC_full_profile eddy_is eddy_it *woa* sigma_* ia ic
%}

%find CCUC profiles from spicyness
load dCC_full_profile
...
for nor=1:repeats
	...
	save dCC_tmp_prof_for_plot eddy_is eddy_it ppres ia ic tpres std_* sigma_* mean_* *woa*
	load dCC_full_profile eddy_it eddy_is
	...
	load dCC_tmp_prof_for_plot eddy_it eddy_is
	subplot(122)
	plot(eddy_is(:,ia),eddy_it(:,ia),'color',[.5 .5 .5],'linewidth',.1)
	hold on
	plot(mean_s_ac,mean_t_ac,'r','linewidth',2)
...
%Now that we have found good eddies, use the IDs and keep all the data we thew out in the eddy interiors.

tt=nansum(eddy_it,1);
ii=find(tt'>0); 


good_id=unique(eddy_id(ii));
...
for m=1:length(good_id)
	igood=find(eddy_id==good_id(m));
	tmp_is(:,igood)=eddy_is(:,igood);
	tmp_it(:,igood)=eddy_it(:,igood);
end	
...
save dCC_eddy_argo_prof.mat eddy_* *woa* nneg ppres lat lon ia ic tpres
%save dCC_eddy_argo_prof.mat eddy_* *woa* nneg ppres lat lon ia ic tpres std_* sigma_* mean_*
...
save dCC_argo_3d ac_* cc_* ppres xi xi yi
