function argo_radial_composite_km(input_track_file,input_domain,outnam)

warning('off','all')
%
fprintf('\r loading index - ext WARNING ftp off')
...
load /matlab/matlab/argo/v1/eddy_argo_prof_index_v5_fast
%load /matlab/matlab/argo/v1/eddy_argo_prof_index_rad
warning('off','all')
uid=unique(id);
...
for m=1:length(same_prof)
    if eddy_plon(tsame(m))>max(lon) | eddy_plon(tsame(m))<min(lon) | eddy_plat(tsame(m))>max(lat) | eddy_plat(tsame(m))<min(lat)
        same_prof(m)=nan;
...
eval(['save ' outname, '_argo_mask_km_ext_comp.mat ac_* cc_* ppres ri km_x step ff eddy_dist ia ic prop_speed nda ndc eddy_*'])
fprintf('\n')
