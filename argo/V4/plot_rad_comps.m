function plot_rad_comps(input_file,output_dir)
...
for m=1:length(ppres)
    if pmean(eddy_y)>0
        ...
        tt=km_x(find(abs(ac_v(m,iin))==ac_max_speed(m)));
        if any(tt)
            ac_imax(m)=tt(1);
        else
            ac_imax(m)=1;
        end
        tt=km_x(find(cc_v(m,iin)==cc_max_speed(m)));
        if any(tt)
            cc_imax(m)=tt(1);
        else
            cc_imax(m)=1;
        ...
        tt=km_x(find(ac_v(m,iin)==ac_max_speed(m)));
        if any(tt)
            ac_imax(m)=tt(1);
        else
            ac_imax(m)=1;    
        end
        tt=km_x(find(abs(cc_v(m,iin))==cc_max_speed(m)));
        if any(tt)
            cc_imax(m)=tt(1);
        else
            cc_imax(m)=1;
...
dlat=min(eddy_y)-1:max(eddy_y)+1;
dlon=min(eddy_x)-1:max(eddy_x)+1;
...
axis tight
set(dd,'fontsize',20,'LineWidth',2,'TickLength',[.01 .005],'layer','top')
xlabel('lon','fontsize',20,'fontweight','bold')
...
%}
%compare DH and SSH
eddy_dh_srf=eddy_dh_anom(1,:);
nkm_x=0:10:200;

for m=1:length(nkm_x)-1
    ii=find(eddy_dist_km(ia)>=nkm_x(m) & eddy_dist_km(ia)<nkm_x(m+1) & eddy_cyc(ia)==1);
    ssh_ac(m)=pmean(eddy_ssh(ia(ii)));
    dh_ac(m)=100*pmean(eddy_dh_srf(ia(ii)));
    
    ii=find(eddy_dist_km(ic)>=nkm_x(m) & eddy_dist_km(ic)<nkm_x(m+1) & eddy_cyc(ic)==-1);
    ssh_cc(m)=pmean(eddy_ssh(ic(ii)));
    dh_cc(m)=100*pmean(eddy_dh_srf(ic(ii)));
end    

mssh=.5*(ssh_ac-ssh_cc);
mdh=.5*(dh_ac-dh_cc);
...
set(gca,'xlim',[0 150],'xtick',[0:50:200],'ytick',[-40:10:40],'ylim',[-40 40],'fontsize',15,'fontweight','bold','LineWidth',2,'TickLength',[.02 .02],'layer','top')
daspect([1 .75 1])
eval(['print -dpng -r300  ',output_dir '/dh_ssh'])
end
