function argo_radial_comp_box(minlat,maxlat,minlon,maxlon,outname)

warning('off','all')

%fprintf('\r loading index - ext')

load ~/data/eddy/V5/global_tracks_v5_12_weeks.mat id x y k cyc track_jday scale
...
for m=1:length(same_prof)
    if eddy_plon(tsame(m))>maxlon | eddy_plon(tsame(m))<minlon | eddy_plat(tsame(m))>maxlat | eddy_plat(tsame(m))<minlat
        same_prof(m)=nan;
...
%     end
    ...
    eval(['save ' outname, '_argo_comp.mat ac_* cc_* *_woa ppres ri km_x step ff eddy_dist ia ic prop_speed nda ndc id cyc track_jday eddy_*'])
    fprintf('\n')
end
