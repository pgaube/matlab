...
for nor=1:repeats
	...
	for nn=1:length(ac_it(1,:))
		...
		%if length(find(uc_ids==eddy_id(ia(nn))))==0
			if tag_a>=round(.2*length(find(~isnan(ac_it(:,nn))))) & abs(eddy_dist_x(ia(nn))')<1 & abs(eddy_dist_y(ia(nn))')<1
				ac_is(:,nn)=nan;
				ac_it(:,nn)=nan;
			end	
		%end	
...
for nor=1:repeats
	...
	for m=1:length(tbins_a)-1
		...
		std_s_cc(m,:)=prctile(ac_is(ii),[0 75]);
		std_t_cc(m,:)=prctile(ac_it(ii),[0 75]);
	end
	...
	for nn=1:length(ac_it(1,:))
		...
		if tag_a>=round(.2*length(find(~isnan(ac_it(:,nn))))) & abs(eddy_dist_x(ic(nn))')<1 & abs(eddy_dist_y(ic(nn))')<1
			ac_is(:,nn)=nan;
			ac_it(:,nn)=nan;
		end	
	end
