...
for nor=1:repeats
	...
	fl=find(abs(eddy_dist_x(iu))'>.5 | abs(eddy_dist_y(iu))'>.5);
	eddy_it_core(:,fl)=nan;
	eddy_is_core(:,fl)=nan;
	...
	for m=1:length(tbins_a)-1
		...
		std_s_ac(m,:)=prctile(ac_is(ii),[10 90]);
		std_t_ac(m,:)=prctile(ac_it(ii),[10 90]);
	end
	...
	for nn=1:length(ac_it(1,:))
		...
		if length(find(uc_ids==eddy_id(ia(nn))))==0
			if tag_a>=round(.1*length(find(~isnan(ac_it(:,nn))))) & abs(eddy_dist_x(ia(nn))')<1 & abs(eddy_dist_y(ia(nn))')<1
				ac_is(:,nn)=nan;
				ac_it(:,nn)=nan;
			end	
		end	
...
for nor=1:repeats
	...
	for m=1:length(tbins_a)-1
		...
		std_s_cc(m,:)=prctile(ac_is(ii),[10 90]);
		std_t_cc(m,:)=prctile(ac_it(ii),[10 90]);
	end
...
ac_sigma=smooth2d_loess(eddy_ist(:,ia),eddy_dist(ia),ppres,1.5,100,ri,ppres);
ac_sigma=fillnans(ac_sigma);
ac_sigma=cat(2,fliplr(ac_sigma(:,2:end)),ac_sigma);
ac_s=smooth2d_loess(eddy_is(:,ia),eddy_dist(ia),ppres,1.5,100,ri,ppres);
ac_s=fillnans(ac_s);
ac_s=cat(2,fliplr(ac_s(:,2:end)),ac_s);
ac_t=smooth2d_loess(eddy_it(:,ia),eddy_dist(ia),ppres,1.5,100,ri,ppres);
ac_t=fillnans(ac_t);
ac_t=cat(2,fliplr(ac_t(:,2:end)),ac_t);
ac_s_anom=smooth2d_loess(eddy_is_anom(:,ia),eddy_dist(ia),ppres,1.5,100,ri,ppres);
ac_s_anom=fillnans(ac_s_anom);
ac_s_anom=cat(2,fliplr(ac_s_anom(:,2:end)),ac_s_anom);
ac_t_anom=smooth2d_loess(eddy_it_anom(:,ia),eddy_dist(ia),ppres,1.5,100,ri,ppres);
ac_t_anom=fillnans(ac_t_anom);
ac_t_anom=cat(2,fliplr(ac_t_anom(:,2:end)),ac_t_anom);
ac_sigma_anom=smooth2d_loess(eddy_ist_anom(:,ia),eddy_dist(ia),ppres,1.5,100,ri,ppres);
ac_sigma_anom=fillnans(ac_sigma_anom);
ac_sigma_anom=cat(2,fliplr(ac_sigma_anom(:,2:end)),ac_sigma_anom);


fprintf('\r now interpolating cyclones')
cc_sigma=smooth2d_loess(eddy_ist(:,ic),eddy_dist(ic),ppres,1.5,100,ri,ppres);
cc_sigma=fillnans(cc_sigma);
cc_sigma=cat(2,fliplr(cc_sigma(:,2:end)),cc_sigma);
cc_s=smooth2d_loess(eddy_is(:,ic),eddy_dist(ic),ppres,1.5,100,ri,ppres);
cc_s=fillnans(cc_s);
cc_s=cat(2,fliplr(cc_s(:,2:end)),cc_s);
cc_t=smooth2d_loess(eddy_it(:,ic),eddy_dist(ic),ppres,1.5,100,ri,ppres);
cc_t=fillnans(cc_t);
cc_t=cat(2,fliplr(cc_t(:,2:end)),cc_t);
cc_s_anom=smooth2d_loess(eddy_is_anom(:,ic),eddy_dist(ic),ppres,1.5,100,ri,ppres);
cc_s_anom=fillnans(cc_s_anom);
cc_s_anom=cat(2,fliplr(cc_s_anom(:,2:end)),cc_s_anom);
cc_t_anom=smooth2d_loess(eddy_it_anom(:,ic),eddy_dist(ic),ppres,1.5,100,ri,ppres);
cc_t_anom=fillnans(cc_t_anom);
cc_t_anom=cat(2,fliplr(cc_t_anom(:,2:end)),cc_t_anom);
cc_sigma_anom=smooth2d_loess(eddy_ist_anom(:,ic),eddy_dist(ic),ppres,1.5,100,ri,ppres);
cc_sigma_anom=fillnans(cc_sigma_anom);
cc_sigma_anom=cat(2,fliplr(cc_sigma_anom(:,2:end)),cc_sigma_anom);


save pCC_argo_2d ac_* cc_* ppres xi
...
for m=1:length(ppres);
	ac_crl(m,:)=dfdx_m(ac_v(m,:),dx);
end	

ac_spd=sqrt(ac_v.^2);
cc_spd=sqrt(cc_v.^2);

save pCC_argo_2d -append  ac_* cc_* ppres xi
