...
[t_woa_tmp,s_woa_tmp,z_woa_tmp]=WOA_profile(eddy_y,eddy_x,tpres,eddy_month);
save VOCALS_raw_trapped_profiles tmp_* t_* eddy_* s_* tpres ppres lap nneg lat lon
%}
...
save VOCALS_full_profile eddy_is eddy_it *woa* sigma_* ia ic
%}
%}
%}
%find CCUC profiles from spicyness
load VOCALS_raw_profiles
load VOCALS_full_profile
...
for nor=1:repeats
	...
	for m=1:length(tbins_a)-1
		...
		std_s_ac(m,:)=prctile(ac_is(ii),[15 85]);
		std_t_ac(m,:)=prctile(ac_it(ii),[15 85]);
	end
	...
	for nn=1:length(ac_it(1,:))
		...
		if tag_a>=round(.4*length(find(~isnan(ac_it(:,nn))))) & abs(eddy_dist_x(ia(nn))')<1 & abs(eddy_dist_y(ia(nn))')<1
			ac_is(:,nn)=nan;
			ac_it(:,nn)=nan;
		end	
	end
	...
	save VOCALS_tmp_prof_for_plot eddy_is eddy_it ppres ia ic tpres std_* sigma_* mean_* *woa*
	load VOCALS_full_profile eddy_it eddy_is
	...
	load VOCALS_tmp_prof_for_plot eddy_it eddy_is
	subplot(122)
	plot(eddy_is(:,ia),eddy_it(:,ia),'color',[.5 .5 .5],'linewidth',.1)
	hold on
	plot(mean_s_ac,mean_t_ac,'r','linewidth',2)
...
for nor=1:repeats
	...
	for m=1:length(tbins_a)-1
		...
		std_s_cc(m,:)=prctile(ac_is(ii),[15 85]);
		std_t_cc(m,:)=prctile(ac_it(ii),[15 85]);
	end
	...
	for nn=1:length(ac_it(1,:))
		...
		if tag_a>=round(.4*length(find(~isnan(ac_it(:,nn))))) & abs(eddy_dist_x(ic(nn))')<1 & abs(eddy_dist_y(ic(nn))')<1
			ac_is(:,nn)=nan;
			ac_it(:,nn)=nan;
		end	
	end
...
save VOCALS_eddy_argo_prof.mat eddy_* *woa* nneg ppres lat lon ia ic tpres
...
save VOCALS_argo_2d ac_* cc_* ppres xi
...
save VOCALS_argo_2d -append  ac_* cc_* ppres xi

fprintf('\n')
plot_2d_comps_VOCALS
