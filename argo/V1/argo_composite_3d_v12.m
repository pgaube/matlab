...
repeats=5;
hores=linspace(3,2,repeats);
verts=linspace(1,3,length(tpres))';
%verts(tpres<=200)=1;
%verts=ones(size(tpres))';
mults=verts*hores;
mults=3*ones(length(tpres),repeats);
...
for der=1:repeats
	...
	for m=1:length(tpres)
		for n=1:length(tmp_it(1,:))
			...
			if any(ii)
				if abs(tmp_st_anom(m,n))>mults(m,der)*sigma_ist(m)
					tmp_t(ii,n)=nan;
					tmp_s(ii,n)=nan;
					flag_t(ii,n)=1;
					flag_s(ii,n)=1;
				...
				%
				if abs(tmp_is_anom(m,n))>mults(m,der)*sigma_is(m)
					tmp_s(ii,n)=nan;
					flag_s(ii,n)=1;
				end
				if abs(tmp_it_anom(m,n))>mults(m,der)*sigma_it(m)
					tmp_t(ii,n)=nan;
					flag_t(ii,n)=1;
				end
	...
	errorbar(mean_is,-tpres,3*sigma_is,'color',[.5 .5 .5])
	subplot(224)
	errorbar(mean_it,-tpres,3*sigma_it,'color',[.5 .5 .5])
	drawnow
...
save EIO_eddy_argo_prof.mat eddy_* *woa* nneg ppres lat lon ia ic sigma_* tpres
...
save EIO_argo_3d ac_* cc_* ppres xi yi
...
cc_spd=sqrt(cc_u_t.^2+cc_v_t.^2);

save EIO_argo_3d -append  ac_* cc_* ppres xi yi
...
load EIO_eddy_argo_prof eddy_dist_x eddy_dist_y eddy_ist eddy_x eddy_y eddy_id
...
drawnow
print -dpng -r300 figs/EIO_profile_map
...
caxis([-0 10])
colorbar
print -dpng -r300 figs/EIO_horz_tmp_sal
...
axes(cc)
ylabel('^\circ C')
print -dpng -r300 figs/EIO_tmp_anom
...
axes(cc)
ylabel('PSU')
print -dpng -r300 figs/EIO_sal_anom
...
axes(cc)
ylabel('\sigma_\theta (kg m^{-3})')
print -dpng -r300 figs/EIO_sigma_anom
...
ylabel('(m s^{-1})')
warning('on','all')
print -dpng -r300 figs/EIO_chan_fig_5
