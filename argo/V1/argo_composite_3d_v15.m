	...
	for m=1:length(tpres)
		for n=1:length(tmp_it(1,:))
			...
			if any(ii)
				...
				%}
				%{
				if eddy_id(n)<nneg & tmp_is_anom(m,n)<-0.2
					tmp_s(ii,n)=nan;
					flag_s(ii,n)=1;
				end
...
eddy_ist=sw_dens(eddy_is,eddy_it,ppres);
woa_st=sw_dens(s_woa,t_woa,ppres);
...
ac_ss=sw_dens(35*ones(size(ac_sm_t)),ac_t,P);
cc_ss=sw_dens(35*ones(size(cc_sm_t)),cc_t,P);
%
%
%
...
[ac_u_sm,ac_v_sm]=geostro_3d(ac_sm_sigma,km_x,km_y,ppres,ff);
[cc_u_sm,cc_v_sm]=geostro_3d(cc_sm_sigma,km_x,km_y,ppres,ff);
...
ylabel('(m s^{-1})')
subplot(323)
contourf(xi,-ppres,squeeze(ac_v_sm(flf,:,:))',[-2:.01:2]);shading flat
hold on
contour(xi,-ppres,squeeze(ac_sm_sigma(flf,:,:))',[20:.1:30],'k')
line([0 0], [-1000 0])
caxis([-.1 .1])
...
subplot(324)
contourf(xi,-ppres,squeeze(cc_v_sm(flf,:,:))',[-2:.01:2]);shading flat
hold on
contour(xi,-ppres,squeeze(cc_sm_sigma(flf,:,:))',[20:.1:30],'k')
line([0 0], [-1000 0])
caxis([-.1 .1])
...
hold on
contour(xi,-ppres,squeeze(ac_ss(flf,:,:))'-1000,[20:.1:30],'k')
line([0 0], [-1000 0])
axis([-1.4 1.4 -1000 0])
%daspect([1 300 1])
...
hold on
contour(xi,-ppres,squeeze(cc_ss(flf,:,:))'-1000,[20:.1:30],'k')
line([0 0], [-1000 0])
caxis([24 28])
axis([-1.4 1.4 -1000 0])
%daspect([1 300 1])
...
clf
subplot(121)
contourf(xi,-ppres,squeeze(ac_sm_sigma(flf,:,:))'-1000,[20:.1:30]);shading flat
hold on
line([0 0], [-1000 0])
axis([-1.4 1.4 -1000 0])
%daspect([1 300 1])
...
colorbar
subplot(122)
contourf(xi,-ppres,squeeze(cc_sm_sigma(flf,:,:))'-1000,[20:.1:30]);shading flat
hold on
line([0 0], [-1000 0])
caxis([24 28])
axis([-1.4 1.4 -1000 0])
