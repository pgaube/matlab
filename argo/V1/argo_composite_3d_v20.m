...
for m=1:length(tbins)-1
	...
	jj=find((ac_is(ii)-mean_s_ac(m))<=-std_s_ac(m) | (ac_is(ii)-mean_s_ac(m))>3*std_s_ac(m));
	ac_is(ii(jj))=nan;
	...
	jj=find((cc_is(ii)-mean_s_cc(m))<=-std_s_cc(m) | (cc_is(ii)-mean_s_cc(m))>3*std_s_cc(m));
	cc_is(ii(jj))=nan;
end	
...
save VOCALS_eddy_argo_prof.mat eddy_* *woa* nneg ppres lat lon ia ic tpres std_*
plot_profiles
%
...
clf
subplot(121)
contourf(xi,-ppres,squeeze(ac_sigma(flf,:,:))',[2:.1:40]);shading flat
hold on
line([0 0], [-1000 0])
axis([-1.5 1.5 -1000 0])
%daspect([1 300 1])
...
colorbar
subplot(122)
contourf(xi,-ppres,squeeze(cc_sigma(flf,:,:))',[2:.1:40]);shading flat
hold on
line([0 0], [-1000 0])
caxis([24 28])
axis([-1.5 1.5 -1000 0])
