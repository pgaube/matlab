...
for nor=1:repeats
	...
	for nn=1:length(ac_it(1,:))
		...
		if tag_a>=round(.2*length(find(~isnan(ac_it(:,nn))))) %| min(ac_is(:,nn))<33 | length(find(~isnan(ac_it(:,nn))))<30
			ac_is(:,nn)=nan;
			ac_it(:,nn)=nan;
		end	
	end
...
for nor=1:repeats
	...
	for m=1:length(tbins_a)-1
		...
		std_s_cc(m,:)=prctile(ac_is(ii),[1 75]);
		std_t_cc(m,:)=prctile(ac_it(ii),[1 75]);
	end
	...
	for nn=1:length(ac_it(1,:))
		...
		if tag_a>=round(.2*length(find(~isnan(ac_it(:,nn))))) %| length(find(~isnan(ac_it(:,nn))))<30
			ac_is(:,nn)=nan;
			ac_it(:,nn)=nan;
		end	
	end
