...
save VOACALS_raw_profiles tmp_* t_* eddy_* s_* tpres ppres lap nneg lat lon
%}
load VOACALS_raw_profiles
warning('off','all')
...
repeats=3;
hores=linspace(3,2.8,repeats);
verts=linspace(1,2,length(tpres))';
mults=verts*hores;
...
for der=1:repeats
	...
	for m=1:length(tpres)
		for n=1:length(tmp_it(1,:))
			...
			if any(ii)
				%if tmp_it_anom(m,n)<2*sigma_it(m,1) | tmp_it_anom(m,n)>5*sigma_it(m,2);
				if abs(tmp_it_anom(m,n))>mults(m,der)*sigma_it(m)
					tmp_t(ii,n)=nan;
					flag_t(ii,n)=1;
				end
		
				%if tmp_is_anom(m,n)<2*sigma_is(m,1) | tmp_is_anom(m,n)>2*sigma_is(m,2);
	...
	errorbar(mean_is,-tpres,mults(der)*sigma_is,'color',[.5 .5 .5])
	subplot(224)
	errorbar(mean_it,-tpres,mults(der)*sigma_it,'color',[.5 .5 .5])
	drawnow
