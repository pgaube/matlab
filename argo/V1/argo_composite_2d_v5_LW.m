...
%load /matlab/data/eddy/V4/full_tracks/EIO_lat_lon_tracks.mat id nneg lon lat amp
...
save EIO_raw_profiles tmp_* t_* eddy_* s_* tpres ppres lap nneg lat lon
...
[t_woa_tmp,s_woa_tmp,z_woa_tmp]=WOA_profile(eddy_y,eddy_x,tpres,eddy_month);
save EIO_raw_trapped_profiles tmp_* t_* eddy_* s_* tpres ppres lap nneg lat lon
%}

load EIO_raw_profiles
warning('off','all')
repeats=3;
...
save EIO_full_profile eddy_is eddy_it *woa* sigma_* ia ic
%}

%find CCUC profiles from spicyness
load EIO_full_profile
...
for nor=1:repeats
	...
	fl=find(abs(eddy_dist_x(ia))'>1 | abs(eddy_dist_y(ia))'>1);
	eddy_it_core(:,fl)=nan;
	eddy_is_core(:,fl)=nan;
	...
	for m=1:length(tbins_a)-1
		...
		std_s_ac(m,:)=prctile(ac_is(ii),[25 100]);
		std_t_ac(m,:)=prctile(ac_it(ii),[25 100]);
	end
	...
	save EIO_tmp_prof_for_plot eddy_is eddy_it ppres ia ic tpres std_* sigma_* mean_* *woa*
	load EIO_full_profile eddy_it eddy_is
	...
	load EIO_tmp_prof_for_plot eddy_it eddy_is
	subplot(122)
	plot(eddy_is(:,ia),eddy_it(:,ia),'color',[.5 .5 .5],'linewidth',.1)
	hold on
	plot(mean_s_ac,mean_t_ac,'r','linewidth',2)
...
for nor=1:repeats
	...
	tbins_a=15:.2:max(ac_it(:))-.5;	
	for m=1:length(tbins_a)-1
		ii=find(ac_it>=tbins_a(m) & ac_it<tbins_a(m+1));
		mean_t_cc(m)=nanmean(ac_it(ii));
		mean_s_cc(m)=nanmean(ac_is(ii));
		std_s_cc(m,:)=prctile(ac_is(ii),[0 50]);
		std_t_cc(m,:)=prctile(ac_it(ii),[0 50]);
	end
	...
	for nn=1:length(ac_it(1,:))
		...
		if tag_a>=round(.08*length(find(~isnan(ac_it(:,nn))))) & abs(eddy_dist_x(ic(nn))')<1 & abs(eddy_dist_y(ic(nn))')<1
			ac_is(:,nn)=nan;
			ac_it(:,nn)=nan;
		end	
	end
	...
	if(percent_s_retained<30 | percent_t_retained<30)
		break
		break
	end	
end
...
save EIO_eddy_argo_prof.mat eddy_* *woa* nneg ppres lat lon ia ic iu tpres
%save EIO_eddy_argo_prof.mat eddy_* *woa* nneg ppres lat lon ia ic tpres std_* sigma_* mean_*

%}

clear
load EIO_eddy_argo_prof
warning('off','all')
step=.1;
ri=[0:step:2.2];
xi=-2.2:step:2.2;
...
ac_sigma=smooth2d_loess(eddy_ist(:,ia),sig_dist(ia),ppres,2,100,xi,ppres);
ac_s=smooth2d_loess(eddy_is(:,ia),sig_dist(ia),ppres,1.5,100,xi,ppres);
ac_t=smooth2d_loess(eddy_it(:,ia),sig_dist(ia),ppres,1.5,100,xi,ppres);
ac_s_anom=smooth2d_loess(eddy_is_anom(:,ia),sig_dist(ia),ppres,1.5,100,xi,ppres);
ac_t_anom=smooth2d_loess(eddy_it_anom(:,ia),sig_dist(ia),ppres,1.5,100,xi,ppres);
ac_sigma_anom=smooth2d_loess(eddy_ist_anom(:,ia),sig_dist(ia),ppres,1.5,100,xi,ppres);


fprintf('\r now interpolating cyclones')
cc_sigma=smooth2d_loess(eddy_ist(:,ic),sig_dist(ic),ppres,2,100,xi,ppres);
cc_s=smooth2d_loess(eddy_is(:,ic),sig_dist(ic),ppres,1.5,100,xi,ppres);
cc_t=smooth2d_loess(eddy_it(:,ic),sig_dist(ic),ppres,1.5,100,xi,ppres);
cc_s_anom=smooth2d_loess(eddy_is_anom(:,ic),sig_dist(ic),ppres,1.5,100,xi,ppres);
cc_t_anom=smooth2d_loess(eddy_it_anom(:,ic),sig_dist(ic),ppres,1.5,100,xi,ppres);
cc_sigma_anom=smooth2d_loess(eddy_ist_anom(:,ic),sig_dist(ic),ppres,1.5,100,xi,ppres);


save EIO_argo_2d ac_* cc_* ppres xi

%}
fprintf('\r now calculating velocites')
load EIO_argo_2d
warning('off','all')
load EIO_eddy_argo_prof eddy_scale eddy_y
...
ac_sm_v=smooth2d_loess(ac_v,xi,ppres,3,200,xi,ppres);
cc_sm_v=smooth2d_loess(cc_v,xi,ppres,3,200,xi,ppres);

for m=1:length(ppres);
	ac_crl(m,:)=dfdx_m(ac_sm_v(m,:),dx);
	cc_crl(m,:)=dfdx_m(cc_sm_v(m,:),dx);
end	


ac_spd=sqrt(ac_sm_v.^2);
cc_spd=sqrt(cc_sm_v.^2);

save EIO_argo_2d -append  ac_* cc_* ppres xi
