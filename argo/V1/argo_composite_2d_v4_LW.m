...
for nor=1:repeats
	...
	tbins_a=min(ac_it(:)):.2:max(ac_it(:))-1;
	...
	for nn=1:length(ac_it(1,:))
		...
		if tag_a>=round(.5*length(find(~isnan(ac_it(:,nn))))) & abs(eddy_dist_x(ia(nn))')<1 & abs(eddy_dist_y(ia(nn))')<1
			ac_is(:,nn)=nan;
			ac_it(:,nn)=nan;
		end	
	end
...
for nor=1:repeats
	...
	for nn=1:length(ac_it(1,:))
		...
		if tag_a>=round(.5*length(find(~isnan(ac_it(:,nn))))) & abs(eddy_dist_x(ic(nn))')<1 & abs(eddy_dist_y(ic(nn))')<1
			ac_is(:,nn)=nan;
			ac_it(:,nn)=nan;
		end	
	end
...
save EIO_eddy_argo_prof.mat eddy_* *woa* nneg ppres lat lon ia ic tpres
...
cc_sigma=smooth2d_loess(eddy_ist(:,ic),sig_dist(ic),ppres,1.5,100,xi,ppres);
