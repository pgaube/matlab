...
for m=1:length(tpres)
	for n=1:length(tmp_it(1,:))
		...
		if any(ii)
			if tmp_it_anom(m,n)<3*sigma_it(m,1) | tmp_it_anom(m,n)>5*sigma_it(m,2);
				tmp_t(ii,n)=nan;
				flag_t(ii,n)=1;
			end
	
			if tmp_is_anom(m,n)<1*sigma_is(m,1) | tmp_is_anom(m,n)>3*sigma_is(m,2);
				tmp_s(ii,n)=nan;
				flag_s(ii,n)=1;
			end
		end
...
[flag_t,flag_s,flag_dh]=deal(zeros(size(eddy_is)));
...
for m=1:length(eddy_id)
	...
	if length(t)>30 & length(s)>30
		eddy_svan(:,m) = sw_svan(eddy_is(:,m),eddy_it(:,m),ppres);
	end	
end

percent_of_s_interpolated=100*(length(find(flag_s==1))/length(find(isnan(eddy_is))))
percent_of_t_interpolated=100*(length(find(flag_t==1))/length(find(isnan(eddy_it))))
...
for m=1:length(ppres);
	...
	ac_svan(:,:,m)=grid2d_loess(eddy_svan(m,ia)',eddy_dist_x(ia),eddy_dist_y(ia),1,1,xi,yi);
end
...
for m=1:length(ppres);
	...
	cc_svan(:,:,m)=grid2d_loess(eddy_svan(m,ic)',eddy_dist_x(ic),eddy_dist_y(ic),1,1,xi,yi);
end
...
for m=1:length(xi)
for n=1:length(yi)
	svan         = squeeze(ac_svan(m,n,:));
	mean_svan    = 0.5*(svan(2:end,:) + svan(1:end-1,:) );
	top          = svan(1).*ppres(1)*db2Pascal;
	delta_ga     = (mean_svan.*diff(ppres))*db2Pascal;
	ac_dh(m,n,:) = cumsum([ top; delta_ga]);

	svan         = squeeze(cc_svan(m,n,:));
	mean_svan    = 0.5*(svan(2:end,:) + svan(1:end-1,:) );
	top          = svan(1).*ppres(1)*db2Pascal;
	delta_ga     = (mean_svan.*diff(ppres))*db2Pascal;
	cc_dh(m,n,:) = cumsum([ top; delta_ga]);
end
end
