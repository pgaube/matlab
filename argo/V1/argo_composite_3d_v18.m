...
for der=1:repeats
	...
	for m=1:length(tpres)
		for n=1:length(tmp_it(1,:))
			...
			if any(ii)
				if abs(tmp_is_anom(m,n))>3*sigma_is(m)% | tmp_is_anom(m,n)<-.25
					tmp_s(ii,n)=nan;
					flag_s(ii,n)=1;
				end
	...
	percent_s_removed(der)=100*(length(find(flag_s==1))/length(flag_s(:)))
	percent_t_removed(der)=100*(length(find(flag_t==1))/length(flag_t(:)))
...
for m=1:length(ppres)
	std_t(m,:)=prctile(eddy_it(m,:),[25 75]);
	std_s(m,:)=prctile(eddy_is(m,:),[25 75]);
end
...
clf
subplot(321)
contourf(xi,yi,squeeze(cc_t_anom(:,:,ipc)),[-2:.2:2]);axis image;hold on
contour(xi,yi,squeeze(cc_t_anom(:,:,ipc)),[-2:.2:2],'k');axis image
shading flat
colormap(cha_bwr)
...
freezecolors
subplot(322)
contourf(xi,yi,squeeze(ac_t_anom(:,:,ipa)),[-2:.2:2]);axis image;hold on
contour(xi,yi,squeeze(ac_t_anom(:,:,ipa)),[-2:.2:2],'k');axis image
shading flat
colormap(cha_bwr)
line([-2.5 2.5],[0 0],'color','k')
line([0 0],[-2.5 2.5],'color','k')
...
subplot(323)
contourf(xi,yi,squeeze(100*cc_s_anom(:,:,ipc)),[-20:2:20]);axis image;hold on
contour(xi,yi,squeeze(100*cc_s_anom(:,:,ipc)),[-20:2:20],'k');axis image
shading flat
colormap(cha_bwr)
line([-2.5 2.5],[0 0],'color','k')
line([0 0],[-2.5 2.5],'color','k')
...
freezecolors
subplot(324)
contourf(xi,yi,squeeze(100*ac_s_anom(:,:,ipa)),[-20:2:20]);axis image;hold on
contour(xi,yi,squeeze(100*ac_s_anom(:,:,ipa)),[-20:2:20],'k');axis image
shading flat
colormap(cha_bwr)
line([-2.5 2.5],[0 0],'color','k')
line([0 0],[-2.5 2.5],'color','k')
...
title('Geostrophic velocity of anticyclones')
subplot(325)
contourf(xi,-ppres,squeeze(cc_u_t(:,flf,:))',[-2:.01:2]);shading flat
hold on
%contour(xi,-ppres,squeeze(cc_sigma(:,flf,:))',[20:.1:30],'k')
line([0 0], [-1000 0])
caxis([-.1 .1])
