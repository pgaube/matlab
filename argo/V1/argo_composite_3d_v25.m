...
for m=1:length(tbins)-1
	ii=find(ac_it>=tbins(m) & ac_it<tbins(m+1));
	std_s_ac(m,:)=prctile(ac_is(ii),[15 100]);
	std_t_ac(m,:)=prctile(ac_it(ii),[15 100]);
	
	ii=find(cc_it>=tbins(m) & cc_it<tbins(m+1));	
	std_s_cc(m,:)=prctile(cc_is(ii),[15 100]);
	std_t_cc(m,:)=prctile(cc_it(ii),[15 100]);
end	
%15 95
...
for nn=1:length(eddy_it(1,:))
	...
	if tag_c>=round(.2*length(find(~isnan(cc_it(:,nn))))) | length(find(~isnan(cc_it(:,nn))))<75
		cc_is(:,nn)=nan;
		cc_it(:,nn)=nan;
	end	
	if tag_a>=round(.2*length(find(~isnan(ac_it(:,nn))))) | length(find(~isnan(ac_it(:,nn))))<75
		ac_is(:,nn)=nan;
		ac_it(:,nn)=nan;
	end	
end
