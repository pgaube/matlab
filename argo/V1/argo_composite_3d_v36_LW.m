...
for nor=1:repeats
	...
	for nn=1:length(ac_it(1,:))
		...
		for m=1:length(tbins_a)-1
			...
			if tag_a>=round(.5*length(find(~isnan(ac_it(:,nn))))) & abs(eddy_dist_x(ia(nn))')<.5 & abs(eddy_dist_y(ia(nn))')<.5 
				ac_is(:,nn)=nan;
				ac_it(:,nn)=nan;
			end	
	end
...
for nor=1:repeats
	...
	for nn=1:length(ac_it(1,:))
		...
		if tag_a>=round(.5*length(find(~isnan(ac_it(:,nn))))) & abs(eddy_dist_x(ic(nn))')<.5 & abs(eddy_dist_y(ic(nn))')<.5 
			ac_is(:,nn)=nan;
			ac_it(:,nn)=nan;
		end	
	end
