...
ii=find(eddy_spice(41,ia)>=.01 & abs(eddy_dist_x(ia)')<1 & abs(eddy_dist_y(ia))'<1);

iuc_ids = sames(unique(eddy_id(ia(ii))),eddy_id);
...
for nor=1:repeats
	...
	clear mean* std*
	eddy_it_core=eddy_it(:,iuc_ids);
	eddy_is_core=eddy_is(:,iuc_ids);
	...
	for m=1:length(tbins_a)-1
		...
		std_s_ac(m,:)=prctile(ac_is(ii),[10 99]);
		std_t_ac(m,:)=prctile(ac_it(ii),[10 99]);
	end
