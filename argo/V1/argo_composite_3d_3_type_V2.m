...
for nor=1:repeats
	...
	tbins_a=min(uc_it(:))+.2:.2:max(uc_it(:))-.1;
	tbins_a=7:.2:max(uc_it(:))-.1;
	for m=1:length(tbins_a)-1
		ii=find(uc_it>=tbins_a(m) & uc_it<tbins_a(m+1));
	...
	std_s_uc(:,1)=smooth1d_loess(std_s_uc(:,1),[1:length(tbins_a)-1]',30,[1:length(tbins_a)-1]');
	std_s_uc(:,2)=smooth1d_loess(std_s_uc(:,2),[1:length(tbins_a)-1]',30,[1:length(tbins_a)-1]');
	std_t_uc(:,1)=smooth1d_loess(std_t_uc(:,1),[1:length(tbins_a)-1]',30,[1:length(tbins_a)-1]');
	std_t_uc(:,2)=smooth1d_loess(std_t_uc(:,2),[1:length(tbins_a)-1]',30,[1:length(tbins_a)-1]');
	
	mean_t_uc=smooth1d_loess(mean_t_uc,[1:length(tbins_a)-1],30,[1:length(tbins_a)-1]);
	mean_s_uc=smooth1d_loess(mean_s_uc,[1:length(tbins_a)-1],30,[1:length(tbins_a)-1]);
	...
	for nn=1:length(uc_it(1,:))
		...
		for m=1:length(tbins_a)-1
			...
			jj=find(uc_is(ii,nn)<=std_s_uc(m,1) | uc_is(ii,nn)>=std_s_uc(m,2));
			tag_a=tag_a+length(jj);
		end	
			if tag_a>=round(.1*length(find(~isnan(uc_it(:,nn))))) & abs(eddy_dist_x(iu(nn))')<.5 & abs(eddy_dist_y(iu(nn))')<.5 
				uc_is(:,nn)=nan;
				uc_it(:,nn)=nan;
			end	
	end
	...
	percent_s_retained=100*(length(find(~isnan(uc_is)))/sbad_u);
	percent_t_retained=100*(length(find(~isnan(uc_it)))/tbad_u);
	
	save pCC_tmp_prof_for_plot eddy_is eddy_it ppres ia ic iu tpres std_* sigma_* mean_* *woa*
...
for nor=1:repeats
	...
	save pCC_tmp_prof_for_plot eddy_is eddy_it ppres ia iu ic tpres std_* sigma_* mean_* *woa*
	load pCC_full_profile eddy_it eddy_is
...
for nor=1:repeats
	...
	for m=1:length(tbins_a)-1
		...
		std_s_cc(m,:)=prctile(ac_is(ii),[25 75]);
		std_t_cc(m,:)=prctile(ac_it(ii),[25 75]);
	end
...
save pCC_eddy_argo_prof.mat eddy_* *woa* nneg ppres lat lon ia iu ic tpres std_* sigma_* mean_*
