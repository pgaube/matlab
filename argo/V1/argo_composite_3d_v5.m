...
for n=1:length(round_pres(1,:))
	for m=1:length(tpres)
		ii=find(tpres==round_pres(m,n));
		%
		if tpres(ii)<300
			if tmp_it_anom(m,n)<3*sigma_it(ii,1) | tmp_it_anom(m,n)>3*sigma_it(ii,2);
				tmp_t(m,n)=nan;
				flag_t(m,n)=1;
			end
		else
		...
		%
		if tpres(ii)<300
			if tmp_is_anom(m,n)<3*sigma_is(ii,1) | tmp_is_anom(m,n)>3*sigma_is(ii,2);
				tmp_s(m,n)=nan;
				flag_s(m,n)=1;
			end
		else
...
for m=1:length(ppres);
	...
	ac_dh(:,:,m)=grid2d_loess(eddy_dh(m,ia)',eddy_dist_x(ia),eddy_dist_y(ia),1,1,xi,yi);
end
...
for m=1:length(ppres);
	...
	cc_dh(:,:,m)=grid2d_loess(eddy_dh(m,ic)',eddy_dist_x(ic),eddy_dist_y(ic),1,1,xi,yi);
end
...
hold on
contour(xi,-ppres,squeeze(ac_dh(17,:,:))',[0:.1:100],'k')
line([0 0], [-1000 0])
axis([-1.4 1.4 -1000 0])
%daspect([1 300 1])
...
hold on
contour(xi,-ppres,squeeze(cc_dh(17,:,:))',[0:.1:100],'k')
line([0 0], [-1000 0])
caxis([24 28])
axis([-1.4 1.4 -1000 0])
%daspect([1 300 1])
