...
save VOCALS_full_profile eddy_is eddy_it *woa* sigma_*
%}
...
for nor=1:repeats
	...
	clear mean* std*
	eddy_it_core=eddy_it;
	eddy_is_core=eddy_is;
	...
	tbins_a=min(ac_it(:)):.2:max(ac_it(:));
	tbins_c=min(cc_it(:)):.2:max(cc_it(:));
	...
	for m=1:length(tbins_c)-1
		ii=find(cc_it>=tbins_c(m) & cc_it<tbins_c(m+1));
		mean_t_cc(m)=nanmean(cc_it(ii));
		mean_s_cc(m)=nanmean(cc_is(ii));
		std_s_cc(m,:)=prctile(cc_is(ii),[10 90]);
		std_t_cc(m,:)=prctile(cc_it(ii),[10 90]);
	end
	...
	std_s_cc(:,1)=smooth1d_loess(std_s_cc(:,1),[1:length(tbins_c)-1]',30,[1:length(tbins_c)-1]');
	std_s_cc(:,2)=smooth1d_loess(std_s_cc(:,2),[1:length(tbins_c)-1]',30,[1:length(tbins_c)-1]');
	...
	std_t_cc(:,1)=smooth1d_loess(std_t_cc(:,1),[1:length(tbins_c)-1]',30,[1:length(tbins_c)-1]');
	std_t_cc(:,2)=smooth1d_loess(std_t_cc(:,2),[1:length(tbins_c)-1]',30,[1:length(tbins_c)-1]');
	
	mean_t_cc=smooth1d_loess(mean_t_cc,[1:length(tbins_c)-1],30,[1:length(tbins_c)-1]);
	mean_s_cc=smooth1d_loess(mean_s_cc,[1:length(tbins_c)-1],30,[1:length(tbins_c)-1]);
	...
	for nn=1:length(eddy_it(1,:))
		...
		for m=1:length(tbins_c)-1
			ii=find(cc_it(:,nn)>=tbins_c(m) & cc_it(:,nn)<tbins_c(m+1));
			jj=find(cc_is(ii,nn)<=std_s_cc(m,1) | cc_is(ii,nn)>=std_s_cc(m,2));
			tag_c=tag_c+length(jj);
		end
		if tag_c>=round(.1*length(find(~isnan(cc_it(:,nn))))) & abs(eddy_dist_x((nn))')<.5 & abs(eddy_dist_y((nn))')<.5 %| length(find(~isnan(cc_it(:,nn))))<30
			cc_is(:,nn)=nan;
			cc_it(:,nn)=nan;
		end	
		if tag_a>=round(.1*length(find(~isnan(ac_it(:,nn))))) & abs(eddy_dist_x((nn))')<.5 & abs(eddy_dist_y((nn))')<.5%| length(find(~isnan(ac_it(:,nn))))<30
			ac_is(:,nn)=nan;
			ac_it(:,nn)=nan;
		end	
	end
