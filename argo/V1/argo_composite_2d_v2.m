...
for nor=1:repeats
	...
	for nn=1:length(ac_it(1,:))
		...
		if tag_a>=round(.3*length(find(~isnan(ac_it(:,nn))))) & abs(eddy_dist_x(ic(nn))')<1 & abs(eddy_dist_y(ic(nn))')<1
			ac_is(:,nn)=nan;
			ac_it(:,nn)=nan;
		end	
	end
...
ac_sigma=smooth2d_loess(eddy_ist(:,ia),sig_dist(ia),ppres,1.5,100,xi,ppres);
