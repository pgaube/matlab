...
for m=1:12
    [bfrq(:,m),vort,p_ave]=sw_bfrq(squeeze(nanmean(squeeze(nanmean(ss(m,:,:),1)),2)),squeeze(nanmean(squeeze(nanmean(tt(m,:,:),1)),2)),z);
end
clf
plot(sqrt(bfrq),p_ave);axis ij;daspect([1 50000 1]);axis([0 .02 0 500])
return
%mask bad data in N
%n(8,1:10,2:40)=0;
%{
