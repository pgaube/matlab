...
% 
% % load ~/data/eddy/V5/new_lw_p_lat_lon_orgin_tracks x y cyc id track_jday scale k
load ~/matlab/regions/tracks/tight/lw_tracks ext_x ext_y cyc id track_jday scale k
ii=find(track_jday>=2451911 & track_jday<=2455137);
x=ext_x(ii);
y=ext_y(ii);
cyc=cyc(ii);
...
%     
    ...
    eval(['fbc=double((',fbc,'));'])
    eval(['fbc2=double((',fbc2,'));'])
    pcomps_raw(fbc,fbc2,[-.1 .1],-1,.01,1,[mon(n,:),' N=',num2str(lab)],1,50)
    ...
    fbc=['lw','_',num2str(n),'_ddwchl_a.median'];
    fbc2=['lw','_',num2str(n),'_wekw_a.median'];
    nn=['lw','_',num2str(n),'_ddwchl_a.n_max_sample'];
    eval(['lab=num2str(', nn,');'])
    eval(['fbc=double((',fbc,'));'])
    eval(['fbc2=double((',fbc2,'));'])
    pcomps_raw(fbc,fbc2,[-.1 .1],-1,.01,1,[mon(n,:),' N=',num2str(lab)],1,50)
    eval(['print -dpng -r300 ~/Documents/OSU/figures/eddy-wind/FINAL_comps/ddwsp66_chl/lw_',num2str(n),'_a']) 

    fbc=['lw','_',num2str(n),'_ddwchl_c.median'];
    fbc2=['lw','_',num2str(n),'_wekw_c.median'];
    nn=['lw','_',num2str(n),'_ddwchl_c.n_max_sample'];
    eval(['fbc=double((',fbc,'));'])
    eval(['fbc2=double((',fbc2,'));'])
    eval(['lab=num2str(', nn,');'])
    pcomps_raw(fbc,fbc2,[-.1 .1],-1,.01,1,[mon(n,:),' N=',num2str(lab)],1,50)
    eval(['print -dpng -r300 ~/Documents/OSU/figures/eddy-wind/FINAL_comps/ddwsp66_chl/lw_',num2str(n),'_c'])
