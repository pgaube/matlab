...
% % % load ~/data/eddy/V5/new_lw_p_lat_lon_orgin_tracks x y cyc id track_jday scale k
% load ~/matlab/regions/tracks/tight/lw_tracks x y cyc id track_jday scale k
% % ii=find(track_jday>=2451911 & track_jday<=2455137 & x<100);
% ii=find(track_jday>=2451911 & track_jday<=2455137);
% x=x(ii);
% y=y(ii);
% cyc=cyc(ii);
...
% fbc=interp2(fbc,2);
% fbc2=interp2(fbc2,2);
% pcomps_raw(fbc,fbc2,[-.004 .004],-1,.01,1,['mean CHL',char(39),' and Ekman pumping'])
...
load double_norm_ek_comps
for n=1:12
    
%     fbc=['lw','_',num2str(n),'_log_a.median'];
...
%     pcomps_raw(fbc,fbc2,[-.02 .02],-1,.01,1,[mon(n,:),' N=',num2str(lab)],1)
%     eval(['print -depsc ~/Documents/OSU/figures/eddy-wind/FINAL_comps/bp26_chl/lw_',num2str(n),'_a']) 
% 
%     fbc=['lw','_',num2str(n),'_log_c.median'];
...
%     eval(['lab=num2str(', nn,');'])
%     pcomps_raw(fbc,fbc2,[-.02 .02],-1,.01,1,[mon(n,:),' N=',num2str(lab)],1)
%     eval(['print -depsc ~/Documents/OSU/figures/eddy-wind/FINAL_comps/bp26_chl/lw_',num2str(n),'_c']) 
%         
%      
...
%     eval(['lab=num2str(', nn,');'])
%     eval(['fbc=double(interp2(',fbc,'));'])
%     pcomps_raw(fbc,fbc,[-.004 .004],-1,.001,1,[mon(n,:),' N=',num2str(lab)])
...
%     eval(['fbc=double(interp2(',fbc,'));'])
%     eval(['lab=num2str(', nn,');'])
%     pcomps_raw(-fliplr(flipud(fbc)),-fliplr(flipud(fbc))+.0005,[-.004 .004],-1,.001,1,[mon(n,:),' N=',num2str(lab)])
...
%     
    ...
    eval(['fbc=double(interp2(',fbc,'));'])
    eval(['fbc2=double(interp2(',fbc2,'));'])
    pcomps_raw(fbc,fbc2,[-.35 .35],-1,.1,1,[mon(n,:),' N=',num2str(lab)],1,30)
    eval(['print -depsc ~/Documents/OSU/figures/eddy-wind/FINAL_comps/dd_sp66_chl/lw_',num2str(n),'_a']) 
    ...
    eval(['lab=num2str(', nn,');'])
    pcomps_raw(fbc,fbc2,[-.35 .35],-1,.1,1,[mon(n,:),' N=',num2str(lab)],1,30)
    eval(['print -depsc ~/Documents/OSU/figures/eddy-wind/FINAL_comps/dd_sp66_chl/lw_',num2str(n),'_c'])
...
%     eval(['fbc2=double(interp2(',fbc2,'));'])
%     eval(['lab=num2str(', nn,');'])
%     pcomps_raw(fbc,fbc2,[-.4 .4],-1,.01,1,[mon(n,:),' N=',num2str(lab)])
