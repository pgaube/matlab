...
path = '/Volumes/matlab/data/QuickScat/from_schlax/';
years = [2002:2005];
...
cwgt=nan(560,1440);
dwgt=nan(560,1440);
SST=nan(length(lat),length(lon),50);
DWGT=nan(length(lat),length(lon),50);
CWGT=nan(length(lat),length(lon),50);
...
for i=1:length(years);
    ...
    if months(j)<10
        filename=[path,'str.',int2str(years(i)),'0',int2str(months(j))];
    end
    if months(j)>=10
        filename=[path,'str.',int2str(years(i)),int2str(months(j))]; 
    end
        if exist(filename)
